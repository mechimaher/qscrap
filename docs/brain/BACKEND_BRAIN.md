...
### Garage & B2B
### 📦 Core Services
- **OrderService**: Atomic order creation with `SERIALIZABLE` isolation and **Suspension Shielding**.
- **PayoutService**: Financial engine with **Dispute-Aware Gatekeeping** and **Auditable Clawbacks**.
- **AddressService**: Premium address management for Qatar (Doha, Al Wakrah, Lusail).
- **SystemController**: Centralized configuration management for secure API key delivery.
| Service | Purpose |
|---------|---------|
...
- **JWT + 401 Sentinel**: Global session expiry handling in both Mobile and Web layers.
- **Audit Logs**: Every state change and automatic system adjustment is recorded in `audit_logs`.
- **Secret Isolation**: Sensitive frontend keys (Maps, Stripe) are delivered via dynamic config endpoints.
