...
exports.driverRepository = exports.DriverRepository = void 0;
const db_1 = require("../config/db");
class DriverRepository {
    pool;
    constructor() {
        this.pool = (0, db_1.getWritePool)();
    }
    getClient(client) {
    ...
    async findActiveAssignments(userId, statusFilter) {
        const result = await this.pool.query(`
            SELECT 
    ...
    async findAssignmentById(assignmentId, userId, client) {
    ...
    async findAssignmentForUpdate(assignmentId, userId, client) {
    ...
    async updateAssignmentsLocation(driverId, lat, lng) {
        const result = await this.pool.query(`
    ...
    async updateAssignmentStatus(assignmentId, status, notes, failureReason, client) {
        let updateQuery = `
            UPDATE delivery_assignments SET
                status = $1,
    ...
    async createStatusHistory(orderId, oldStatus, newStatus, userId, reason, client) {
        await client.query(`
    ...
    async createDispute(orderId, customerId, garageId, reason, description, client) {
        await client.query(`
    ...
    async countOtherActiveAssignments(driverId, currentAssignmentId, client) {
        const result = await client.query(`
    ...
    async updateDriverStatus(driverId, status, client) {
        const db = this.getClient(client);
        await db.query(`UPDATE drivers SET status = $1, updated_at = NOW() WHERE driver_id = $2`, [status, driverId]);
    }
    async updateDriverStatusByUserId(userId, status) {
        const result = await this.pool.query(`
            UPDATE drivers SET 
                status = $1,
                updated_at = NOW()
    ...
    async incrementDeliveryCount(driverId, client) {
        await client.query(`
            UPDATE drivers SET 
    ...
    async getOrderTotal(orderId, client) {
        const result = await client.query('SELECT total_amount, order_number FROM orders WHERE order_id = $1', [orderId]);
        return result.rows[0];
    }
    async createPayout(driverId, assignmentId, orderId, orderNumber, amount, client) {
        await client.query(`
    ...
    async updateDriverEarnings(driverId, amount, client) {
        await client.query(`
            UPDATE drivers SET 
    ...
    async saveDeliveryProof(assignmentId, photoUrl, signatureUrl, notes) {
        const result = await this.pool.query(`
            UPDATE delivery_assignments SET
