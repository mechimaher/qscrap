...
exports.getPendingDisputesCount = exports.autoResolveDisputes = exports.garageRespondToDispute = exports.getDisputeDetails = exports.getMyDisputes = exports.createDispute = void 0;
const db_1 = __importDefault(require("../config/db"));
// Dispute reason configurations with refund rules
const DISPUTE_CONFIGS = {
...
const createDispute = async (req, res) => {
    ...
    try {
        ...
        // Get order details
        const orderResult = await client.query(`SELECT o.*, 
        ...
        const existingDispute = await client.query(`SELECT dispute_id FROM disputes WHERE order_id = $1`, [order_id]);
        ...
        // Create dispute
        const disputeResult = await client.query(`INSERT INTO disputes 
             (order_id, customer_id, garage_id, reason, description, photo_urls, 
              refund_amount, restocking_fee)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
             RETURNING dispute_id, created_at`, [order_id, customerId, order.garage_id, reason, description,
            photoUrls, refundAmount, restockingFee]);
        // Update order status
        await client.query(`UPDATE orders SET order_status = 'disputed', updated_at = NOW() WHERE order_id = $1`, [order_id]);
...
const getMyDisputes = async (req, res) => {
    ...
    try {
        ...
        // Count query
        const countResult = await db_1.default.query(`SELECT COUNT(*) FROM disputes d ${whereClause}`, params);
        const total = parseInt(countResult.rows[0].count);
        const totalPages = Math.ceil(total / limitNum);
        // Main query with pagination
        const result = await db_1.default.query(`SELECT d.*, o.order_number, o.part_price, o.total_amount,
...
const getDisputeDetails = async (req, res) => {
    ...
    const userId = req.user.userId;
    try {
        const result = await db_1.default.query(`SELECT d.*, o.order_number, o.part_price, o.total_amount, o.delivery_fee,
                    pr.car_make, pr.car_model, pr.part_description,
             ...
             WHERE d.dispute_id = $1 
               AND (d.customer_id = $2 OR d.garage_id = $2)`, [dispute_id, userId]);
...
const garageRespondToDispute = async (req, res) => {
    const { dispute_id } = req.params;
    ...
    try {
        await client.query('BEGIN');
        // Get dispute
        const disputeResult = await client.query(`SELECT d.*, o.order_number, o.customer_id
             FROM disputes d
             JOIN orders o ON d.order_id = o.order_id
             WHERE d.dispute_id = $1 FOR UPDATE`, [dispute_id]);
        ...
        await client.query(`UPDATE disputes 
             SET status = 'under_review', 
                 garage_response = $2,
                 updated_at = NOW()
             WHERE dispute_id = $1`, [dispute_id, response_message]);
        await client.query('COMMIT');
...
const autoResolveDisputes = async () => {
    try {
        ...
        for (const dispute of result.rows) {
            await db_1.default.query(`UPDATE orders SET order_status = 'refunded' WHERE order_id = $1`, [dispute.order_id]);
            // Notify customer
            const io = global.io;
...
exports.autoResolveDisputes = autoResolveDisputes;
// Get pending disputes count for garage
const getPendingDisputesCount = async (req, res) => {
    const garageId = req.user.userId;
    try {
        const result = await db_1.default.query(`SELECT COUNT(*) as count FROM disputes WHERE garage_id = $1 AND status = 'pending'`, [garageId]);
...
exports.getPendingDisputesCount = getPendingDisputesCount;
