...
exports.withdrawBid = exports.updateBid = exports.rejectBid = exports.getMyBids = exports.submitBid = void 0;
...
const validateWarrantyDays = (days) => {
...
const submitBid = async (req, res) => {
    ...
    console.log('[BID] Submit request files:', req.files ? req.files.length : 0);
    ...
    try {
        ...
        // ============================================
        const planCheck = await client.query(`SELECT COALESCE(sp.plan_code, 'starter') as plan_code
             FROM garages g
        ...
        const existingBid = await client.query('SELECT bid_id FROM bids WHERE request_id = $1 AND garage_id = $2', [targetRequestId, garageId]);
        if (existingBid.rows.length > 0) {
            throw new Error('You have already submitted a bid for this request');
        }
        // Check subscription limits
        const subCheck = await client.query(`SELECT sp.max_bids_per_month, gs.bids_used_this_cycle, gs.status
        ...
        // Insert Bid
        const bidResult = await client.query(`INSERT INTO bids 
       (request_id, garage_id, bid_amount, warranty_days, notes, part_condition, brand_name, part_number, image_urls)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING bid_id, created_at`, [targetRequestId, garageId, amountCheck.value, validatedWarranty, notes, part_condition, brand_name, part_number, image_urls]);
        // Update Request Count
        await client.query(`UPDATE part_requests SET bid_count = bid_count + 1 WHERE request_id = $1`, [targetRequestId]);
        ...
        const bidCountResult = await client.query('SELECT COUNT(*) FROM bids WHERE request_id = $1', [targetRequestId]);
        ...
        try {
            global.io.to(`user_${customerId}`).emit('new_bid', {
...
const getMyBids = async (req, res) => {
    ...
    try {
        ...
        const countResult = await db_1.default.query(`SELECT COUNT(*) FROM bids b ${whereClause}`, params);
        res.json({
            bids: result.rows,
            pagination: {
                page: pageNum,
...
const rejectBid = async (req, res) => {
    ...
    try {
        const check = await db_1.default.query(`SELECT b.bid_id, b.garage_id, b.status, pr.car_make, pr.car_model 
             FROM bids b
             JOIN part_requests pr ON b.request_id = pr.request_id
             WHERE b.bid_id = $1 AND pr.customer_id = $2`, [bid_id, userId]);
        ...
        await db_1.default.query("UPDATE bids SET status = 'rejected', updated_at = NOW() WHERE bid_id = $1", [bid_id]);
        global.io?.to(`garage_${bid.garage_id}`).emit('bid_rejected', {
            bid_id,
...
const updateBid = async (req, res) => {
    ...
    try {
        const check = await db_1.default.query(`SELECT b.*, pr.customer_id, pr.car_make, pr.car_model
             FROM bids b
             JOIN part_requests pr ON b.request_id = pr.request_id
             WHERE b.bid_id = $1 AND b.garage_id = $2`, [bid_id, garageId]);
        if (check.rows.length === 0) {
        ...
        await db_1.default.query(`UPDATE bids SET 
                ...
                updated_at = NOW()
             WHERE bid_id = $6`, [bid_amount, warranty_days, notes, part_condition, brand_name, bid_id]);
        global.io?.to(`user_${bid.customer_id}`).emit('bid_updated', {
...
const withdrawBid = async (req, res) => {
    ...
    try {
        ...
        await db_1.default.query("UPDATE bids SET status = 'withdrawn', updated_at = NOW() WHERE bid_id = $1", [bid_id]);
        await db_1.default.query('UPDATE part_requests SET bid_count = GREATEST(0, bid_count - 1) WHERE request_id = $1', [bid.request_id]);
