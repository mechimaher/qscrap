...
exports.getGaragesReport = exports.getDeliveriesReport = exports.getDisputesReport = exports.getRevenueReport = exports.getOrdersReport = void 0;
...
const getOrdersReport = async (req, res) => {
    try {
        ...
        const ordersResult = await db_1.default.query(`SELECT o.order_id, o.order_number, o.order_status, o.payment_status,
        ...
        const summaryResult = await db_1.default.query(`SELECT 
             ...
             ${summaryStatusFilter}`, summaryParams);
        res.json({
            report_type: 'orders',
...
const getRevenueReport = async (req, res) => {
    try {
        ...
        const toDate = to || new Date().toISOString().split('T')[0];
        // Daily breakdown
        const dailyResult = await db_1.default.query(`SELECT 
             ...
             ORDER BY DATE(created_at)`, [fromDate, toDate]);
        // Top performing garages
        const topGaragesResult = await db_1.default.query(`SELECT g.garage_name, g.garage_id,
        ...
        const summaryResult = await db_1.default.query(`SELECT 
             ...
             FROM orders
             WHERE created_at >= $1 AND created_at <= $2::date + interval '1 day'
               AND order_status NOT IN ('cancelled_by_customer', 'cancelled_by_garage', 'cancelled_by_ops')`, [fromDate, toDate]);
        res.json({
            report_type: 'revenue',
            period: { from: fromDate, to: toDate },
            generated_at: new Date().toISOString(),
...
// Disputes Report
const getDisputesReport = async (req, res) => {
    try {
        const { from, to } = req.query;
        const fromDate = from || new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0];
        const toDate = to || new Date().toISOString().split('T')[0];
        const disputesResult = await db_1.default.query(`SELECT d.dispute_id, d.reason, d.description, d.resolution,
             ...
             ORDER BY d.created_at DESC`, [fromDate, toDate]);
        const summaryResult = await db_1.default.query(`SELECT 
             ...
             FROM disputes
             WHERE created_at >= $1 AND created_at <= $2::date + interval '1 day'`, [fromDate, toDate]);
        res.json({
...
exports.getDisputesReport = getDisputesReport;
// Deliveries Report
const getDeliveriesReport = async (req, res) => {
    try {
        const { from, to } = req.query;
        const fromDate = from || new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0];
        const toDate = to || new Date().toISOString().split('T')[0];
        // Driver performance
        const driversResult = await db_1.default.query(`SELECT u.full_name as driver_name, u.user_id as driver_id,
             ...
             ORDER BY total_deliveries DESC`, [fromDate, toDate]);
        // Delivery summary
        const summaryResult = await db_1.default.query(`SELECT 
...
exports.getDeliveriesReport = getDeliveriesReport;
// Garages Performance Report
const getGaragesReport = async (req, res) => {
    try {
        const { from, to } = req.query;
        const fromDate = from || new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0];
        const toDate = to || new Date().toISOString().split('T')[0];
        const garagesResult = await db_1.default.query(`SELECT g.garage_id, g.garage_name, g.rating_average, g.rating_count, g.total_transactions,
...
exports.getGaragesReport = getGaragesReport;
