...
exports.getMarketInsights = exports.getPlanFeatures = exports.getCustomerInsights = exports.exportAnalytics = exports.getGarageAnalytics = void 0;
const db_1 = __importDefault(require("../config/db"));
// Helper: Get garage's plan features
async function getGaragePlanFeatures(garageId) {
    const result = await db_1.default.query(`SELECT COALESCE(sp.plan_code, 'starter') as plan_code,
...
function hasAnalyticsAccess(analyticsLevel) {
...
const getGarageAnalytics = async (req, res) => {
    ...
    try {
        ...
        const [revenueData, bidStats, orderStats, categoryStats, ratingTrend] = await Promise.all([
            // Revenue by day
            db_1.default.query(`SELECT DATE(o.completed_at) as date,
                        COUNT(*) as orders,
            ...
            db_1.default.query(`SELECT 
                 ...
                 FROM bids
                 WHERE garage_id = $1
                   AND created_at >= CURRENT_DATE - INTERVAL '${days} days'`, [garageId]),
            // Order stats
            db_1.default.query(`SELECT 
                 ...
                 WHERE garage_id = $1 
                   ...
                   AND completed_at >= CURRENT_DATE - INTERVAL '${days} days'`, [garageId]),
            // Top categories (from bid data)
            db_1.default.query(`SELECT pr.part_description as part_name, 
                 ...
                 LIMIT 5`, [garageId]),
            // Rating trend
            db_1.default.query(`SELECT DATE(r.created_at) as date,
...
exports.getGarageAnalytics = getGarageAnalytics;
...
const exportAnalytics = async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.default.query(`SELECT o.order_id, o.created_at, o.completed_at, o.order_status,
             ...
             WHERE o.garage_id = $1
               AND o.completed_at >= CURRENT_DATE - INTERVAL '${days} days'
             ORDER BY o.completed_at DESC`, [garageId]);
        res.json({
            export_type: 'orders',
            period_days: days,
    ...
    try {
        ...
        const [repeatCustomers, topCustomers, areaBreakdown] = await Promise.all([
            ...
            db_1.default.query(`SELECT 
                 ...
                 FROM orders
                 WHERE garage_id = $1 AND order_status = 'completed'`, [garageId]),
            ...
            db_1.default.query(`SELECT 
                    ...
                    SUM(o.garage_payout_amount) as revenue
                 FROM orders o
                 WHERE o.garage_id = $1 AND o.order_status = 'completed'`, [garageId])
        ]);
...
const getMarketInsights = async (req, res) => {
    ...
    try {
        ...
        const [platformStats, garageRanking, myStats, marketAverages, trendingParts] = await Promise.all([
            // Platform-wide stats
            db_1.default.query(`SELECT 
        ...
        const winRateResult = await db_1.default.query(`SELECT 
                COUNT(*) as total_bids,
                COUNT(*) FILTER (WHERE status = 'accepted') as won_bids
             FROM bids
             WHERE garage_id = $1
               AND created_at >= CURRENT_DATE - INTERVAL '30 days'`, [garageId]);
        ...
        const marketWinRateResult = await db_1.default.query(`SELECT 
                COUNT(*) as total_bids,
                COUNT(*) FILTER (WHERE status = 'accepted') as won_bids
             FROM bids
             WHERE created_at >= CURRENT_DATE - INTERVAL '30 days'`);
