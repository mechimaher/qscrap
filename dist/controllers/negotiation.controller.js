...
exports.acceptLastGarageOffer = exports.getPendingCounterOffers = exports.getNegotiationHistory = exports.customerRespondToCounter = exports.respondToCounterOffer = exports.createCounterOffer = void 0;
...
const createCounterOffer = async (req, res) => {
    ...
    try {
        ...
        // Get the bid
        const bidResult = await client.query(`SELECT b.*, pr.customer_id 
             FROM bids b 
        ...
        // Check round limit
        const roundResult = await client.query(`SELECT COUNT(*) as count FROM counter_offers WHERE bid_id = $1`, [bid_id]);
        ...
        const pendingResult = await client.query(`SELECT counter_offer_id FROM counter_offers 
             WHERE bid_id = $1 AND status = 'pending'`, [bid_id]);
        if (pendingResult.rows.length > 0) {
            throw new Error('A pending counter-offer already exists');
        }
        // Create counter-offer
        const result = await client.query(`INSERT INTO counter_offers 
             (bid_id, request_id, offered_by_type, offered_by_id, proposed_amount, message, round_number)
             VALUES ($1, $2, 'customer', $3, $4, $5, $6)
             RETURNING counter_offer_id, created_at`, [bid_id, bid.request_id, customerId, proposed_amount, message, currentRound]);
...
const respondToCounterOffer = async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        // Get counter-offer
        const coResult = await client.query(`SELECT co.*, b.garage_id, b.bid_amount, b.request_id, pr.customer_id
             FROM counter_offers co
             JOIN bids b ON co.bid_id = b.bid_id
             JOIN part_requests pr ON b.request_id = pr.request_id
             WHERE co.counter_offer_id = $1`, [counter_offer_id]);
        ...
        if (action === 'accept') {
            ...
            await client.query(`UPDATE counter_offers SET status = 'accepted', response_message = $2, responded_at = NOW()
                 WHERE counter_offer_id = $1`, [counter_offer_id, message]);
            // Update bid amount to the agreed price
            await client.query(`UPDATE bids SET bid_amount = $1, notes = COALESCE(notes, '') || ' [Negotiated from ' || bid_amount || ' QAR]', updated_at = NOW()
                 WHERE bid_id = $2`, [co.proposed_amount, co.bid_id]);
            await client.query('COMMIT');
            // Notify customer
            const io = global.io;
            ...
            await client.query(`UPDATE counter_offers SET status = 'rejected', response_message = $2, responded_at = NOW()
                 WHERE counter_offer_id = $1`, [counter_offer_id, message]);
            await client.query('COMMIT');
            // Notify customer
            const io = global.io;
            ...
            await client.query(`UPDATE counter_offers SET status = 'countered', response_message = $2, responded_at = NOW()
                 WHERE counter_offer_id = $1`, [counter_offer_id, message]);
            // Create new counter-offer from garage
            const newCo = await client.query(`INSERT INTO counter_offers 
                 (bid_id, request_id, offered_by_type, offered_by_id, proposed_amount, message, round_number)
                 VALUES ($1, $2, 'garage', $3, $4, $5, $6)
                 RETURNING counter_offer_id`, [co.bid_id, co.request_id, garageId, counter_amount, message, co.round_number + 1]);
            await client.query('COMMIT');
            // Notify customer
            const io = global.io;
...
const customerRespondToCounter = async (req, res) => {
    const { counter_offer_id } = req.params;
    ...
    try {
        ...
        const coResult = await client.query(`SELECT co.*, b.garage_id, b.bid_amount, b.request_id, pr.customer_id
             ...
             JOIN bids b ON co.bid_id = b.bid_id
             JOIN part_requests pr ON b.request_id = pr.request_id
             WHERE co.counter_offer_id = $1 AND co.offered_by_type = 'garage'`, [counter_offer_id]);
        if (coResult.rows.length === 0) {
            throw new Error('Counter-offer not found');
        }
        const co = coResult.rows[0];
        ...
        if (action === 'accept') {
            await client.query(`UPDATE counter_offers SET status = 'accepted', response_message = $2, responded_at = NOW()
                 WHERE counter_offer_id = $1`, [counter_offer_id, message]);
            // Update bid amount
            await client.query(`UPDATE bids SET bid_amount = $1, notes = COALESCE(notes, '') || ' [Negotiated]', updated_at = NOW()
                 WHERE bid_id = $2`, [co.proposed_amount, co.bid_id]);
            await client.query('COMMIT');
            // Notify garage
            const io = global.io;
            ...
            const newCo = await client.query(`INSERT INTO counter_offers 
                 (bid_id, request_id, offered_by_type, offered_by_id, proposed_amount, message, round_number)
                 VALUES ($1, $2, 'customer', $3, $4, $5, $6)
                 RETURNING counter_offer_id`, [co.bid_id, co.request_id, customerId, counter_amount, message, co.round_number + 1]);
            await client.query('COMMIT');
            const io = global.io;
...
exports.customerRespondToCounter = customerRespondToCounter;
...
const getNegotiationHistory = async (req, res) => {
    ...
    try {
        // Verify access
        const bidResult = await db_1.default.query(`SELECT b.garage_id, pr.customer_id FROM bids b 
             JOIN part_requests pr ON b.request_id = pr.request_id 
             WHERE b.bid_id = $1`, [bid_id]);
        if (bidResult.rows.length === 0) {
        ...
        const result = await db_1.default.query(`SELECT * FROM counter_offers 
             WHERE bid_id = $1 
             ORDER BY created_at ASC`, [bid_id]);
        res.json({
...
const getPendingCounterOffers = async (req, res) => {
    const garageId = req.user.userId;
    try {
        const result = await db_1.default.query(`SELECT co.*, b.bid_amount as original_amount, 
             ...
             ORDER BY co.created_at DESC`, [garageId]);
...
exports.getPendingCounterOffers = getPendingCounterOffers;
/**
 ...
 */
const acceptLastGarageOffer = async (req, res) => {
    const { bid_id } = req.params;
    const customerId = req.user.userId;
    const client = await db_1.default.connect();
    try {
        ...
        const bidResult = await client.query(`SELECT b.*, pr.customer_id, pr.request_id,
        ...
        await client.query(`UPDATE bids SET bid_amount = $1, updated_at = NOW() WHERE bid_id = $2`, [finalAmount, bid_id]);
        // Mark any pending counter-offers as expired
        await client.query(`UPDATE counter_offers SET status = 'accepted', responded_at = NOW()
             WHERE bid_id = $1 AND status = 'pending'`, [bid_id]);
        await client.query('COMMIT');
        // Notify garage
        const io = global.io;
...
exports.acceptLastGarageOffer = acceptLastGarageOffer;
