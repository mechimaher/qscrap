...
exports.deleteAccount = exports.login = exports.register = void 0;
...
const register = async (req, res) => {
    const { phone_number, password, user_type, full_name, garage_name, address, 
    // Garage specialization fields
    supplier_type, specialized_brands, all_brands } = req.body;
    ...
    try {
        ...
        const hash = await bcrypt_1.default.hash(password, security_1.BCRYPT_ROUNDS);
        // Insert User
        const userResult = await client.query('INSERT INTO users (phone_number, password_hash, user_type, full_name) VALUES ($1, $2, $3, $4) RETURNING user_id', [phone_number, hash, user_type, full_name]);
        ...
        if (user_type === 'garage') {
            ...
            demoExpiresAt.setDate(demoExpiresAt.getDate() + security_1.TRIAL_DAYS);
            // Create garage with auto demo trial and specialization
            await client.query(`INSERT INTO garages (garage_id, garage_name, address, approval_status, demo_expires_at,
                                      supplier_type, specialized_brands, all_brands) 
                 VALUES ($1, $2, $3, 'demo', $4, $5, $6, $7)`, [userId, garage_name, address, demoExpiresAt,
                supplier_type || 'used',
                specialized_brands || [],
                all_brands !== false]);
        ...
        const token = jsonwebtoken_1.default.sign({ userId, userType: user_type }, (0, security_1.getJwtSecret)(), { expiresIn: security_1.TOKEN_EXPIRY_SECONDS });
...
const login = async (req, res) => {
    ...
    try {
        const result = await db_1.default.query('SELECT * FROM users WHERE phone_number = $1', [phone_number]);
        ...
        if (user.user_type === 'garage') {
            const garageResult = await db_1.default.query(`SELECT approval_status, demo_expires_at, rejection_reason 
                 FROM garages WHERE garage_id = $1`, [user.user_id]);
            if (garageResult.rows.length > 0) {
                ...
                if (approvalStatus === 'demo' && garage.demo_expires_at) {
                    ...
                    if (demoExpiry < new Date()) {
                        ...
                        await db_1.default.query(`UPDATE garages SET approval_status = 'expired' WHERE garage_id = $1`, [user.user_id]);
                        // Allow login but with expired flag (grace period for upgrade)
                        const token = jsonwebtoken_1.default.sign({ userId: user.user_id, userType: user.user_type }, (0, security_1.getJwtSecret)(), { expiresIn: security_1.TOKEN_EXPIRY_SECONDS });
                        await db_1.default.query('UPDATE users SET last_login_at = NOW() WHERE user_id = $1', [user.user_id]);
                        return res.json({
                            token,
    ...
    try {
        ...
        await client.query(`UPDATE users 
             SET phone_number = $1, 
                 ...
                 fcm_token = NULL
             WHERE user_id = $3`, [anonymizedPhone, anonymizedName, userId]);
        if (userResult.rows[0].user_type === 'garage') {
            await client.query(`UPDATE garages 
                 SET approval_status = 'rejected', 
                     garage_name = $1 
                 WHERE garage_id = $2`, [`Deleted Garage ${userId}`, userId]);
        }
        await client.query('COMMIT');
