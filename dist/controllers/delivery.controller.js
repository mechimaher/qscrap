...
exports.updateZoneFee = exports.getDeliveryZones = exports.calculateDeliveryFee = exports.getActiveDeliveries = exports.updateDriverLocation = exports.getDeliveryStats = exports.updateDeliveryStatus = exports.reassignDriver = exports.assignDriver = exports.getOrdersForDelivery = exports.collectOrder = exports.getOrdersReadyForDelivery = exports.getOrdersReadyForCollection = exports.updateDriver = exports.createDriver = exports.getDriverDetails = exports.getDrivers = void 0;
exports.getDeliveryFeeForLocation = getDeliveryFeeForLocation;
...
const getDriverDetails = async (req, res) => {
    ...
    try {
        const driverResult = await db_1.default.query('SELECT * FROM drivers WHERE driver_id = $1', [driver_id]);
        if (driverResult.rows.length === 0) {
            return res.status(404).json({ error: 'Driver not found' });
        }
        // Get recent assignments
        const assignmentsResult = await db_1.default.query(`
...
const updateDriver = async (req, res) => {
    ...
    try {
        ...
        if (status === 'available') {
            const activeDelivery = await db_1.default.query(`SELECT o.order_id, o.order_number, o.order_status 
                 FROM orders o 
                 WHERE o.driver_id = $1 
...
const getOrdersReadyForCollection = async (req, res) => {
    try {
        const result = await db_1.default.query(`
...
exports.getOrdersReadyForCollection = getOrdersReadyForCollection;
...
exports.getOrdersReadyForDelivery = getOrdersReadyForDelivery;
...
const collectOrder = async (req, res) => {
    ...
    try {
        ...
        const orderResult = await client.query(`SELECT o.order_id, o.order_number, o.order_status, o.customer_id, o.garage_id,
        ...
        if (driver_id) {
            ...
            const driverResult = await client.query('SELECT driver_id, user_id, status, full_name, phone, vehicle_type, vehicle_plate FROM drivers WHERE driver_id = $1', [driver_id]);
            if (driverResult.rows.length === 0) {
            ...
            await client.query('UPDATE drivers SET status = $1, updated_at = NOW() WHERE driver_id = $2', ['busy', driver_id]);
            // Update order with driver (orders.driver_id references users.user_id)
            await client.query('UPDATE orders SET driver_id = $1, updated_at = NOW() WHERE order_id = $2', [driver.user_id, order_id]);
        ...
        await client.query('UPDATE orders SET order_status = $1, updated_at = NOW() WHERE order_id = $2', ['collected', order_id]);
        ...
        if (driver_id && assignedDriver) {
            await client.query('UPDATE drivers SET status = $1, total_deliveries = total_deliveries + 1, updated_at = NOW() WHERE driver_id = $2', ['available', driver_id]);
...
exports.collectOrder = collectOrder;
...
exports.getOrdersForDelivery = getOrdersForDelivery;
...
const assignDriver = async (req, res) => {
    ...
    try {
        ...
        await client.query('UPDATE orders SET order_status = $1, driver_id = $2, updated_at = NOW() WHERE order_id = $3', ['in_transit', driver.user_id, order_id]);
...
const reassignDriver = async (req, res) => {
    ...
    try {
        ...
        const newDriverResult = await client.query('SELECT driver_id, user_id, status, full_name, phone, vehicle_type, vehicle_plate FROM drivers WHERE driver_id = $1', [new_driver_id]);
        ...
        await client.query('UPDATE orders SET driver_id = $1, updated_at = NOW() WHERE order_id = $2', [newDriver.user_id, assignment.order_id]);
        // Set OLD driver status back to available
        if (assignment.old_driver_id) {
            await client.query('UPDATE drivers SET status = $1, updated_at = NOW() WHERE driver_id = $2', ['available', assignment.old_driver_id]);
        }
        // Set NEW driver status to busy
        await client.query('UPDATE drivers SET status = $1, updated_at = NOW() WHERE driver_id = $2', ['busy', new_driver_id]);
...
const updateDeliveryStatus = async (req, res) => {
    ...
    try {
        ...
        if (status === 'delivered') {
            await client.query('UPDATE orders SET order_status = $1, delivered_at = NOW(), updated_at = NOW() WHERE order_id = $2', ['delivered', assignment.order_id]);
            // Add to status history
            await client.query(`
                INSERT INTO order_status_history (order_id, old_status, new_status, changed_by, changed_by_type, reason)
            ...
            // Free up driver
            await client.query('UPDATE drivers SET status = $1, total_deliveries = total_deliveries + 1, updated_at = NOW() WHERE driver_id = $2', ['available', assignment.driver_id]);
        }
        await client.query('COMMIT');
...
const getDeliveryStats = async (req, res) => {
    try {
        const statsResult = await db_1.default.query(`
            SELECT 
...
const updateDriverLocation = async (req, res) => {
    ...
    try {
        ...
        const orderResult = await db_1.default.query('SELECT customer_id, order_number FROM orders WHERE order_id = $1', [assignment.order_id]);
        if (orderResult.rows.length > 0) {
            const order = orderResult.rows[0];
            const io = global.io;
...
const calculateDeliveryFee = async (req, res) => {
    ...
    try {
        ...
        const distance = calculateDistance(hub.latitude, hub.longitude, parseFloat(latitude), parseFloat(longitude));
        // Find matching zone based on distance
        const zoneResult = await db_1.default.query(`
        ...
        if (zoneResult.rows.length === 0) {
            ...
            const defaultZone = await db_1.default.query(`
                SELECT zone_id, zone_name, delivery_fee, min_distance_km, max_distance_km
...
const getDeliveryZones = async (_req, res) => {
    try {
        const result = await db_1.default.query(`
...
const updateZoneFee = async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const currentResult = await client.query('SELECT delivery_fee FROM delivery_zones WHERE zone_id = $1', [zone_id]);
...
exports.updateZoneFee = updateZoneFee;
// Get delivery fee for an order (internal use - called from order.controller.ts)
async function getDeliveryFeeForLocation(lat, lng) {
    const hub = await getPrimaryHub();
