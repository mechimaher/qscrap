...
exports.getCancellationHistory = exports.cancelOrderByGarage = exports.cancelOrderByCustomer = exports.getCancellationPreview = exports.withdrawBid = exports.cancelRequest = void 0;
...
const cancelRequest = async (req, res) => {
    ...
    try {
        ...
        const reqResult = await client.query(`SELECT * FROM part_requests 
             WHERE request_id = $1 AND customer_id = $2
             FOR UPDATE`, [request_id, customerId]);
...
const withdrawBid = async (req, res) => {
    ...
    try {
        ...
        const bidResult = await client.query(`SELECT b.*, pr.customer_id 
             ...
             FOR UPDATE OF b`, [bid_id, garageId]);
        ...
        await client.query(`UPDATE bids 
             SET status = 'withdrawn', 
                 withdrawal_reason = $2,
                 withdrawn_at = NOW()
             WHERE bid_id = $1`, [bid_id, reason || 'Garage withdrew bid']);
        // Decrement request bid count
        await client.query(`UPDATE part_requests 
             SET bid_count = GREATEST(0, bid_count - 1)
             WHERE request_id = $1`, [bid.request_id]);
        await client.query('COMMIT');
        // Notify customer
        global.io.to(`user_${bid.customer_id}`).emit('bid_withdrawn', {
...
const getCancellationPreview = async (req, res) => {
    ...
    try {
        const orderResult = await db_1.default.query(`SELECT * FROM orders WHERE order_id = $1 AND (customer_id = $2 OR garage_id = $2)`, [order_id, userId]);
...
exports.getCancellationPreview = getCancellationPreview;
...
const cancelOrderByCustomer = async (req, res) => {
    ...
    try {
        ...
        // Lock and get order
        const orderResult = await client.query(`SELECT * FROM orders WHERE order_id = $1 AND customer_id = $2 FOR UPDATE`, [order_id, customerId]);
        if (orderResult.rows.length === 0) {
        ...
        const cancelResult = await client.query(`INSERT INTO cancellation_requests 
             ...
             RETURNING cancellation_id`, [order_id, customerId, reason_code || 'changed_mind', reason_text,
        ...
        await client.query(`INSERT INTO order_status_history 
             ...
             VALUES ($1, $2, 'cancelled_by_customer', $3, 'customer', $4)`, [order_id, order.order_status, customerId, reason_text]);
        ...
        if (refundAmount > 0 && order.payment_status === 'paid') {
            await client.query(`INSERT INTO refunds 
                 ...
                 VALUES ($1, $2, $3, $4, $5, 'pending')`, [order_id, cancelResult.rows[0].cancellation_id, order.total_amount, refundAmount, feeInfo.fee]);
        }
        await client.query('COMMIT');
        // Notify garage (use garage_ room)
        global.io.to(`garage_${order.garage_id}`).emit('order_cancelled', {
            order_id,
...
const cancelOrderByGarage = async (req, res) => {
    const { order_id } = req.params;
    const { reason_code, reason_text } = req.body;
    const garageId = req.user.userId;
    const client = await db_1.default.connect();
    try {
        await client.query('BEGIN ISOLATION LEVEL SERIALIZABLE');
        // Lock and get order
        const orderResult = await client.query(`SELECT * FROM orders WHERE order_id = $1 AND garage_id = $2 FOR UPDATE`, [order_id, garageId]);
        if (orderResult.rows.length === 0) {
            throw new Error('Order not found or access denied');
        }
        const order = orderResult.rows[0];
        ...
        const cancelResult = await client.query(`INSERT INTO cancellation_requests 
             ...
             RETURNING cancellation_id`, [order_id, garageId, reason_code || 'stock_out', reason_text,
        ...
        await client.query(`INSERT INTO order_status_history 
             (order_id, old_status, new_status, changed_by, changed_by_type, reason)
             VALUES ($1, $2, 'cancelled_by_garage', $3, 'garage', $4)`, [order_id, order.order_status, garageId, reason_text]);
        // Update garage fulfillment rate
        await client.query(`UPDATE garages 
             ...
             WHERE garage_id = $1`, [garageId]);
        // Full refund for customer
        if (order.payment_status === 'paid') {
            await client.query(`INSERT INTO refunds 
                 (order_id, cancellation_id, original_amount, refund_amount, fee_retained, refund_status)
                 VALUES ($1, $2, $3, $3, 0, 'pending')`, [order_id, cancelResult.rows[0].cancellation_id, order.total_amount]);
        }
        await client.query('COMMIT');
        // Notify customer
        global.io.to(`user_${order.customer_id}`).emit('order_cancelled', {
            order_id,
            order_number: order.order_number,
            cancelled_by: 'garage',
...
exports.cancelOrderByGarage = cancelOrderByGarage;
...
const getCancellationHistory = async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.default.query(`SELECT cr.*, o.order_number, o.total_amount as order_total
