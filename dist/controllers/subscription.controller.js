...
exports.getPaymentHistory = exports.cancelSubscription = exports.changePlan = exports.subscribeToPlan = exports.getMySubscription = exports.getSubscriptionPlans = void 0;
const db_1 = __importDefault(require("../config/db"));
// Get all available subscription plans
const getSubscriptionPlans = async (req, res) => {
    try {
        const result = await db_1.default.query(`SELECT plan_id, plan_code, plan_name, plan_name_ar, monthly_fee, 
...
const getMySubscription = async (req, res) => {
    ...
    try {
        ...
        const bidCountResult = await db_1.default.query(`SELECT COUNT(*) as bids_this_month 
             FROM bids 
             WHERE garage_id = $1 
        ...
        const result = await db_1.default.query(`SELECT gs.*, sp.plan_code, sp.plan_name, sp.monthly_fee, 
        ...
        const garageResult = await db_1.default.query(`SELECT approval_status, demo_expires_at 
             FROM garages 
...
const subscribeToPlan = async (req, res) => {
    ...
    try {
        ...
        const existingSub = await client.query(`SELECT subscription_id FROM garage_subscriptions 
             WHERE garage_id = $1 AND status IN ('active', 'trial')`, [garageId]);
        if (existingSub.rows.length > 0) {
            throw new Error('Already have an active subscription. Please cancel or upgrade instead.');
        }
        // Get plan details
        const planResult = await client.query(`SELECT * FROM subscription_plans WHERE plan_code = $1 AND is_active = true`, [plan_code]);
        ...
        // Create subscription
        const subResult = await client.query(`INSERT INTO garage_subscriptions 
             (garage_id, plan_id, status, billing_cycle_start, billing_cycle_end, next_billing_date)
             VALUES ($1, $2, 'active', $3, $4, $4)
             RETURNING subscription_id`, [garageId, plan.plan_id, today.toISOString().split('T')[0], cycleEnd.toISOString().split('T')[0]]);
        ...
        await client.query(`INSERT INTO subscription_payments 
             ...
             VALUES ($1, $2, $3, 'completed', NOW())`, [subscriptionId, plan.monthly_fee, payment_method || 'card']);
...
const changePlan = async (req, res) => {
    ...
    try {
        ...
        const currentSub = await client.query(`SELECT gs.*, sp.monthly_fee as current_fee, sp.plan_name as current_plan
             FROM garage_subscriptions gs
             JOIN subscription_plans sp ON gs.plan_id = sp.plan_id
        ...
        // Get new plan
        const newPlan = await client.query(`SELECT * FROM subscription_plans WHERE plan_code = $1 AND is_active = true`, [new_plan_code]);
        ...
        await client.query(`UPDATE garage_subscriptions 
             SET plan_id = $1, updated_at = NOW()
             WHERE subscription_id = $2`, [newP.plan_id, current.subscription_id]);
        await client.query('COMMIT');
        res.json({
...
const cancelSubscription = async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const result = await client.query(`UPDATE garage_subscriptions 
             ...
             RETURNING subscription_id, billing_cycle_end`, [garageId, reason]);
        if (result.rows.length === 0) {
...
const getPaymentHistory = async (req, res) => {
    const garageId = req.user.userId;
    try {
        const result = await db_1.default.query(`SELECT sp.*, gs.billing_cycle_start, gs.billing_cycle_end
