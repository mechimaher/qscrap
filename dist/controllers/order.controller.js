...
exports.getGarageReviews = exports.submitReview = exports.confirmDelivery = exports.getOrderDetails = exports.getMyOrders = exports.updateOrderStatus = exports.acceptBid = void 0;
...
const getGarageCommissionRate = async (garageId) => {
    // First check if garage is in demo mode
    const garageResult = await db_1.default.query(`SELECT approval_status FROM garages WHERE garage_id = $1`, [garageId]);
    ...
    const result = await db_1.default.query(`SELECT sp.commission_rate 
         ...
         ORDER BY gs.created_at DESC LIMIT 1`, [garageId]);
...
const acceptBid = async (req, res) => {
    ...
    try {
        ...
        // Lock Bid
        const bidResult = await client.query('SELECT * FROM bids WHERE bid_id = $1 FOR UPDATE', [bid_id]);
        ...
        // Lock Request
        const reqResult = await client.query('SELECT * FROM part_requests WHERE request_id = $1 FOR UPDATE', [bid.request_id]);
        ...
        const negotiatedPriceResult = await client.query(`SELECT 
                COALESCE(
                    ...
                    $2
                ) as final_price`, [bid_id, bid.bid_amount]);
        ...
        if (request.delivery_lat && request.delivery_lng) {
            const zoneInfo = await (0, delivery_controller_1.getDeliveryFeeForLocation)(parseFloat(request.delivery_lat), parseFloat(request.delivery_lng));
        ...
        const orderResult = await client.query(`INSERT INTO orders 
             ...
             RETURNING order_id, order_number`, [bid.request_id, bid_id, customerId, bid.garage_id, part_price, commissionRate,
        ...
        await client.query("UPDATE bids SET status = 'rejected', updated_at = NOW() WHERE request_id = $1 AND bid_id != $2 AND status = 'pending'", [bid.request_id, bid_id]);
        ...
        await client.query(`INSERT INTO order_status_history 
             ...
             VALUES ($1, NULL, 'confirmed', $2, 'customer', 'Order created from accepted bid')`, [order.order_id, customerId]);
        // Update garage transaction count
        await client.query(`UPDATE garages SET total_transactions = total_transactions + 1, updated_at = NOW() WHERE garage_id = $1`, [bid.garage_id]);
        ...
        global.io.to(`user_${customerId}`).emit('order_created', {
            order_id: order.order_id,
            order_number: order.order_number,
            total_amount,
        ...
        const rejectedBids = await db_1.default.query(`SELECT DISTINCT garage_id FROM bids WHERE request_id = $1 AND bid_id != $2`, [bid.request_id, bid_id]);
        rejectedBids.rows.forEach((r) => {
            global.io.to(`garage_${r.garage_id}`).emit('bid_rejected', {
                request_id: bid.request_id,
...
const updateOrderStatus = async (req, res) => {
    ...
    try {
        ...
        const check = await client.query('SELECT customer_id, order_status, order_number FROM orders WHERE order_id = $1 AND garage_id = $2 FOR UPDATE', [order_id, garageId]);
        ...
        await client.query(`UPDATE orders SET ${updateFields.join(', ')} WHERE order_id = $2`, updateValues);
        // Log status change
        await client.query(`INSERT INTO order_status_history 
             (order_id, old_status, new_status, changed_by, changed_by_type, reason)
             VALUES ($1, $2, $3, $4, 'garage', $5)`, [order_id, oldStatus, order_status, garageId, notes]);
        await client.query('COMMIT');
        // Notify customer
        ...
        global.io.to(`user_${customerId}`).emit('order_status_updated', {
            order_id,
        ...
        if (order_status === 'ready_for_pickup') {
            global.io.to('operations').emit('order_ready_for_pickup', {
                order_id,
                order_number: currentOrder.order_number,
...
const getOrderDetails = async (req, res) => {
    ...
    const userId = req.user.userId;
    try {
        const result = await db_1.default.query(`SELECT o.*, 
                    pr.car_make, pr.car_model, pr.car_year, pr.part_description, pr.image_urls as request_images,
             ...
             WHERE o.order_id = $1 AND (o.customer_id = $2 OR o.garage_id = $2)`, [order_id, userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Order not found' });
        }
        // Get status history
        const history = await db_1.default.query(`SELECT * FROM order_status_history WHERE order_id = $1 ORDER BY created_at ASC`, [order_id]);
        // Get review if exists
        const review = await db_1.default.query(`SELECT * FROM order_reviews WHERE order_id = $1`, [order_id]);
        res.json({
            order: result.rows[0],
...
const confirmDelivery = async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(`UPDATE orders 
             ...
             RETURNING garage_id, order_number, garage_payout_amount`, [order_id, customerId]);
        if (result.rows.length === 0) {
            await client.query('ROLLBACK');
        ...
        await client.query(`INSERT INTO order_status_history 
             (order_id, old_status, new_status, changed_by, changed_by_type, reason)
             VALUES ($1, 'delivered', 'completed', $2, 'customer', 'Customer confirmed receipt')`, [order_id, customerId]);
        // Create payout record for garage (skip if already exists from dispute/re-delivery)
        await client.query(`INSERT INTO garage_payouts 
             ...
             AND NOT EXISTS (SELECT 1 FROM garage_payouts gp WHERE gp.order_id = o.order_id)`, [order_id]);
        ...
        global.io.to(`garage_${order.garage_id}`).emit('order_completed', {
            order_id,
            order_number: order.order_number,
        ...
        global.io.to('operations').emit('order_status_updated', {
            order_id,
            order_number: order.order_number,
        ...
        global.io.to('operations').emit('payout_pending', {
            order_id,
            order_number: order.order_number,
...
const submitReview = async (req, res) => {
    ...
    try {
        ...
        const orderCheck = await client.query(`SELECT garage_id FROM orders 
             WHERE order_id = $1 AND customer_id = $2 AND order_status = 'completed'`, [order_id, customerId]);
        ...
        await client.query(`INSERT INTO order_reviews 
             ...
             ON CONFLICT (order_id) DO UPDATE SET
                ...
                is_visible = false,
                updated_at = NOW()`, [order_id, customerId, garageId, overall_rating, part_quality_rating,
            communication_rating, delivery_rating, review_text]);
        await client.query('COMMIT');
        // Notify Operations about new review pending moderation
        global.io.to('operations').emit('new_review_pending', {
            order_id,
            garage_id: garageId,
            overall_rating,
...
const getGarageReviews = async (req, res) => {
    ...
    try {
        const result = await db_1.default.query(`SELECT r.*, u.full_name as customer_name
        ...
        const stats = await db_1.default.query(`SELECT 
             ...
             FROM order_reviews
             WHERE garage_id = $1 AND is_visible = true`, [garage_id]);
        res.json({
            reviews: result.rows,
            stats: stats.rows[0]
        });
...
exports.getGarageReviews = getGarageReviews;
