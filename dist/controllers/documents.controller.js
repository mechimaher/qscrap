...
exports.verifyDocument = exports.downloadDocument = exports.getMyDocuments = exports.getOrderDocuments = exports.getDocument = exports.generateInvoice = void 0;
...
const generateInvoice = async (req, res) => {
    ...
    try {
        ...
        const existingDoc = await db_1.default.query(`
        ...
        const docNumberResult = await db_1.default.query(`SELECT generate_document_number('invoice') as doc_number`);
        const docNumber = docNumberResult.rows[0].doc_number;
        const verifyCodeResult = await db_1.default.query(`SELECT generate_verification_code() as verify_code`);
        ...
        if (QRCode) {
            try {
                qrCodeData = await QRCode.toDataURL(`https://qscrap.qa/verify/${verifyCode}`, { width: 150, margin: 1 });
            }
            catch (qrErr) {
        ...
        const digitalSignature = crypto_1.default
            .createHash('sha256')
...
const getOrderDocuments = async (req, res) => {
    ...
    try {
        ...
        const orderCheck = await db_1.default.query(`
...
const getMyDocuments = async (req, res) => {
    ...
    try {
        ...
        `;
        const params = userType !== 'admin' && userType !== 'operations' ? [userId] : [];
        if (type) {
...
function getLogoBase64() {
    try {
        const logoPath = path_1.default.join(__dirname, '../../public/assets/images/qscrap-logo.png');
...
// ============================================
function generateBilingualCustomerInvoiceHTML(data, qrCode, logoBase64 = '') {
    const formatDate = (dateStr) => {
        const d = new Date(dateStr);
        return d.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' });
    };
    const formatMoney = (n) => n.toLocaleString('en-QA', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
// ============================================
function generateGaragePayoutStatementHTML(data, qrCode, logoBase64 = '') {
    const formatDate = (dateStr) => {
        const d = new Date(dateStr);
        return d.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' });
    };
...
function generateInvoiceHTML(data, qrCode) {
    const formatDate = (dateStr) => {
        const d = new Date(dateStr);
        return d.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' });
    };
...
function formatConditionBilingual(condition) {
    const conditions = {
        'new': { en: 'New', ar: 'جديد' },
...
async function logDocumentAccess(documentId, action, actorId, actorType, req) {
    try {
        await db_1.default.query(`
