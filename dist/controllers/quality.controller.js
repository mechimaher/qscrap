...
exports.getInspectionReport = exports.createReturnAssignment = exports.getQCPassedOrders = exports.getQCStats = exports.getInspectionHistory = exports.submitInspection = exports.startInspection = exports.getPendingInspections = exports.getReadyForCollection = exports.getInspectionCriteria = void 0;
const db_1 = __importDefault(require("../config/db"));
// Get inspection criteria checklist
const getInspectionCriteria = async (req, res) => {
    try {
...
const getReadyForCollection = async (req, res) => {
    try {
        const result = await db_1.default.query(`
...
exports.getReadyForCollection = getReadyForCollection;
...
const getPendingInspections = async (req, res) => {
    ...
    try {
        ...
        const countResult = await db_1.default.query(`
            SELECT COUNT(*) FROM orders WHERE order_status = 'collected'
        `);
...
exports.getPendingInspections = getPendingInspections;
...
const startInspection = async (req, res) => {
    ...
    try {
        ...
        const existing = await client.query('SELECT inspection_id, status FROM quality_inspections WHERE order_id = $1', [order_id]);
        ...
        // Update order status to QC in progress
        await client.query('UPDATE orders SET order_status = $1, updated_at = NOW() WHERE order_id = $2', ['qc_in_progress', order_id]);
...
const submitInspection = async (req, res) => {
    const { order_id } = req.params;
    const { result, checklist_results, notes, failure_reason, failure_category, photo_urls, part_grade, condition_assessment, item_notes } = req.body;
    const inspector_id = req.user?.userId;
    ...
    try {
        ...
        await client.query('UPDATE orders SET order_status = $1, updated_at = NOW() WHERE order_id = $2', [newOrderStatus, order_id]);
...
const getInspectionHistory = async (req, res) => {
    ...
    try {
        // Get total count
        const countResult = await db_1.default.query(`
            SELECT COUNT(*) FROM quality_inspections WHERE status IN ('passed', 'failed')
        `);
        const total = parseInt(countResult.rows[0].count);
        const totalPages = Math.ceil(total / limit);
        // Get paginated results
...
exports.getQCStats = getQCStats;
// Get orders that passed QC and are ready for driver assignment
const getQCPassedOrders = async (req, res) => {
    try {
        const result = await db_1.default.query(`
...
exports.getQCPassedOrders = getQCPassedOrders;
// Create return assignment for QC-failed parts
const createReturnAssignment = async (req, res) => {
    const { order_id } = req.params;
    ...
    try {
        ...
        await client.query('UPDATE orders SET order_status = $1, updated_at = NOW() WHERE order_id = $2', ['returning_to_garage', order_id]);
...
exports.createReturnAssignment = createReturnAssignment;
