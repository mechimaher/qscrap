...
// Get my disputes (for customer or garage)
...
export const garageRespondToDispute = async (req: AuthRequest, res: Response) => {
    const { dispute_id } = req.params;
    const { action, response_message } = req.body; // action: 'accept' | 'contest'
    ...
    try {
        ...
        if (action === 'accept') {
            // Garage accepts the return - process refund
            await client.query(
                `UPDATE disputes 
                 SET status = 'accepted', 
                     garage_response = $2,
            ...
            // Update order status
            await client.query(
                `UPDATE orders SET order_status = 'refunded', updated_at = NOW() WHERE order_id = $1`,
                [dispute.order_id]
            );
            ...
            // Notify customer
            const io = (global as any).io;
            io.to(`user_${dispute.customer_id}`).emit('dispute_accepted', {
                dispute_id: dispute_id,
                order_number: dispute.order_number,
                refund_amount: dispute.refund_amount,
                notification: `✅ Your dispute for Order #${dispute.order_number} was accepted. Refund: ${dispute.refund_amount} QAR`
            });

            res.json({
                message: 'Dispute accepted. Customer will receive refund.',
                refund_amount: dispute.refund_amount
            });

        } else if (action === 'contest') {
            // Garage contests the dispute
            await client.query(
                `UPDATE disputes 
            ...
            // Notify customer
            const io = (global as any).io;
            io.to(`user_${dispute.customer_id}`).emit('dispute_contested', {
                dispute_id: dispute_id,
                order_number: dispute.order_number,
                garage_response: response_message,
                notification: `⚠️ Garage contested your dispute for Order #${dispute.order_number}. Platform will review.`
            });

            res.json({
                message: 'Dispute contested. Platform will review within 24-48 hours.',
                status: 'under_review'
            });
