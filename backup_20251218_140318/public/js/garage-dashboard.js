                            ...
                            <div class="bid-header" style="margin-bottom: 8px;">
                                <div>
                                    ...
                                    <div style="color: var(--text-secondary); font-size: 13px; margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                        ${b.car_summary || 'Vehicle'} - ${b.part_description?.slice(0, 30) || 'Part'}
                                    </div>
                                </div>
    ...
    try {
        ...
        const html = orders.length ? orders.map(o => {
            ...
            if (isDisputed) {
                ...
                // Completed
                actionsHtml = `<div class="order-actions"><span style="color: var(--success);"><i class="bi bi-check-circle-fill"></i> Order completed</span></div>`;
            } else if (isRefunded) {
                // Refunded orders
...
async function reportPaymentIssue(payoutId, amount) {
    ...
    try {
        ...
        if (res.ok) {
            showToast('⚠️ Issue reported. Operations will review and contact you.', 'warning');
            loadEarnings();
        } else {
...
async function loadPayoutHistory() {
    ...
    try {
        const res = await fetch(`${API_URL}/finance/payouts`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await res.json();
