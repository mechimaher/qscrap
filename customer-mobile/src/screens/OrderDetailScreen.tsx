            ...
            <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
                ...
                {order.order_status === 'delivered' && (
                    <View style={{ gap: Spacing.md, marginBottom: Spacing.lg }}>
                        <TouchableOpacity
                            style={[styles.confirmButton, isConfirming && { opacity: 0.7 }, { marginBottom: 0 }]}
                        ...
                        {/* GOD-LEVEL DISPUTE SENTINEL: Direct escalation for delivered orders */}
                        <TouchableOpacity
                            style={styles.disputeButton}
                            onPress={() => {
                                Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
                                Alert.alert(
                                    t('dispute.title') || 'Report an Issue',
                                    t('dispute.confirmMessage') || 'Are you sure there is an issue with this part? This will freeze the payment and notify our resolution team.',
                                    [
                                        { text: t('common.cancel'), style: 'cancel' },
                                        { 
                                            text: t('dispute.report') || 'Report Issue', 
                                            style: 'destructive',
                                            onPress: () => {
                                                navigation.navigate('SupportChat' as any, {
                                                    order_id: order.order_id,
                                                    order_number: order.order_number,
                                                    subject: 'DISPUTE: Incorrect/Damaged Part',
                                                    auto_message: `🚨 DISPUTE INITIATED for Order #${order.order_number}. Customer reports an issue with the delivered part.`
                                                });
                                            }
                                        }
                                    ]
                        ...
                        >
                            ...
                            <Text style={styles.disputeText}>{t('order.reportIssue') || 'Report an Issue / Wrong Part'}</Text>
                        </TouchableOpacity>
                    </View>
                )}
...
const styles = StyleSheet.create({
    ...
    disputeIcon: { fontSize: 18, marginRight: Spacing.sm },
    disputeText: { fontSize: FontSizes.md, fontWeight: '600', color: '#4B5563' },
});
