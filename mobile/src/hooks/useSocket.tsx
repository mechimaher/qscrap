    ...
    const connect = useCallback(async (isRetry = false) => {
        try {
            ...
            socket.current.on('disconnect', (reason) => {
                ...
                const ts = Date.now();
                lastDisconnectAtRef.current = ts;
                setLastDisconnectAt(ts);
                setLastError(reason);
                scheduleReconnect();
            });
            ...
            socket.current.on('new_bid', (data: BidNotification) => {
                ...
                import('../services/notifications').then(({ scheduleLocalNotification }) => {
                    ...
                    const body = data.warranty_days
                        ? t('notifications.push.newBidBodyWarranty', {
                              garage: data.garage_name,
                              condition: conditionLabel,
                              days: data.warranty_days
                          })
            ...
            socket.current.on('bid_updated', (data: { bid_id: string; bid_amount?: number; request_id?: string }) => {
                ...
                if (data.bid_amount !== undefined && data.bid_amount !== null) {
                    ...
                    setNewBids((prev) =>
                        prev
                            .map((bid) => (bid.bid_id === data.bid_id ? { ...bid, bid_amount: data.bid_amount! } : bid))
            ...
            socket.current.on('garage_counter_offer', (data: any) => {
                ...
                import('../services/notifications').then(({ scheduleLocalNotification }) => {
                    ...
                    scheduleLocalNotification(
                        ...
                        data.notification ||
                            t('notifications.push.counterOfferBody', { amount: data.proposed_amount, currency }),
                        {
                            type: 'counter_offer',
                            bidId: data.bid_id,
            ...
            socket.current.on('counter_offer_accepted', (data: any) => {
                ...
                import('../services/notifications').then(({ scheduleLocalNotification }) => {
                    ...
                    scheduleLocalNotification(
                        ...
                        data.notification ||
                            t('notifications.push.offerAcceptedBody', { amount: data.agreed_amount, currency }),
                        {
                ...
                if (data.is_final_round) {
                    ...
                    import('../services/notifications').then(({ scheduleLocalNotification }) => {
                        ...
                        scheduleLocalNotification(
                            ...
                            data.notification ||
                                t('notifications.push.finalOfferBody', { amount: data.original_bid_amount, currency }),
                            {
            ...
            socket.current.on(
                ...
                (data: OrderStatusUpdate & { garage_name?: string; part_description?: string }) => {
                    ...
                    if (statusInfo) {
                        import('../services/notifications').then(({ scheduleLocalNotification }) => {
                            scheduleLocalNotification(`QScrap | ${statusInfo.title}`, statusInfo.body, {
                                type: 'order_update',
            ...
            socket.current.on(
                'order_cancelled',
                (data: { order_id: string; order_number?: string; reason?: string; cancelled_by?: string }) => {
                    log('[Socket] Order cancelled:', data.order_id);
                    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);

                    import('../services/notifications').then(({ scheduleLocalNotification }) => {
                        const cancelledBy =
                            data.cancelled_by === 'garage'
                        ...
                        scheduleLocalNotification(
                            ...
                            data.reason
                                ...
                                : t('notifications.push.orderCancelledBody', {
