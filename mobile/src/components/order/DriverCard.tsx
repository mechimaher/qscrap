            ...
            >
                <View style={styles.driverInfo}>
                    ...
                    <View style={isRTL ? { marginRight: Spacing.md } : { marginLeft: Spacing.md }}>
                        <Text style={[styles.driverLabel, { color: colors.textSecondary, textAlign: rtlTextAlign(isRTL) }]}>{t('common.yourDriver')}</Text>
                        <Text style={[styles.driverName, { color: colors.text, textAlign: rtlTextAlign(isRTL) }]}>{order.driver_name}</Text>
                    </View>
                    {
                        order.driver_phone && (
                            <TouchableOpacity style={[styles.callButton, { marginLeft: isRTL ? 0 : 'auto', marginRight: isRTL ? 'auto' : 0 }]} onPress={onCall}>
                                <Ionicons name="call" size={16} color="#fff" style={isRTL ? { marginLeft: Spacing.xs } : { marginRight: Spacing.xs }} />
...
const styles = StyleSheet.create({
    ...
    driverAvatar: { width: 50, height: 50, borderRadius: 25, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center', marginEnd: Spacing.md, ...Shadows.sm },
    ...
    callIcon: { fontSize: 16, marginEnd: Spacing.xs },
