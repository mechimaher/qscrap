...
const QUICK_REPLY_KEYS: Record<
    string,
    Array<{ id: string; translationKey: string; emoji: string; category: string }>
> = {
    ...
    forGarage: [
        {
            id: '1',
            translationKey: 'quickReplies.partStillAvailable',
    ...
    forCustomer: [
        ...
        {
            id: '8',
            translationKey: 'quickReplies.deliveryComplete',
 ...
 */
export const QuickReplies: React.FC<QuickRepliesProps> = ({ recipientType, onSelectReply }) => {
    ...
    const getReplies = (): QuickReply[] => {
        const keySet =
            recipientType === 'driver'
                ? QUICK_REPLY_KEYS.forDriver
                : recipientType === 'garage'
                  ? QUICK_REPLY_KEYS.forGarage
    ...
    const filteredReplies = selectedCategory ? replies.filter((r) => r.category === selectedCategory) : replies;
        ...
        <View style={styles.container}>
                ...
                {categories.map((cat) => (
                    ...
                    >
                        <Ionicons
                            name={cat.icon}
                            size={14}
                            color={selectedCategory === cat.key ? Colors.primary : '#525252'}
                            style={{ marginRight: 4 }}
                        />
                        <Text
            ...
            >
                {filteredReplies.map((reply) => (
                    <TouchableOpacity
                        key={reply.id}
                        style={[styles.replyChip, { borderColor: CATEGORY_COLORS[reply.category] + '40' }]}
                        onPress={() => handleSelect(reply)}
                        activeOpacity={0.7}
                    >
                        <Ionicons
                            name={reply.emoji as any}
                            size={16}
                            color={CATEGORY_COLORS[reply.category]}
