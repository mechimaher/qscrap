...
// ==========================================
// REAL-TIME COUNTER-OFFER & DISPUTE HANDLERS
// ==========================================

/**
 * Load pending counter-offers from bids and show modal if any exist
 */
async function loadPendingCounterOffers() {
    try {
        // Check for pending counter-offers from existing bids
        const res = await fetch(`${API_URL}/bids/my`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await res.json();
        ...
        // Find bids with pending customer counter-offers awaiting garage response
        const pendingOffers = bids.filter(b =>
            b.has_pending_counter && b.latest_offer_by === 'customer'
        );

        if (pendingOffers.length > 0) {
            const offer = pendingOffers[0];
            // Auto-open modal for the first pending counter-offer
            openCounterRespondModal(
                offer.latest_counter_id,
                offer.bid_amount,
                offer.latest_offer_amount,
                offer.latest_offer_message || 'Customer proposed a new price'
            );
        }
    } catch (err) {
        console.error('Failed to load pending counter-offers:', err);
...
/**
 * Open modal to respond to customer's counter-offer
 */
function openCounterRespondModal(counterOfferId, originalAmount, proposedAmount, message) {
    // Create modal HTML dynamically
    const existingModal = document.getElementById('counterRespondModal');
    if (existingModal) existingModal.remove();

    const savings = originalAmount - proposedAmount;
    const savingsPercent = ((savings / originalAmount) * 100).toFixed(1);

    const modalHtml = `
        <div id="counterRespondModal" class="modal" style="display: flex;">
            <div class="modal-content" style="max-width: 500px;">
                <div class="modal-header">
                    <h3><i class="bi bi-chat-dots-fill text-primary"></i> Customer Counter-Offer</h3>
                    <button onclick="closeCounterRespondModal()" class="close-btn">&times;</button>
                </div>
                <div class="modal-body">
                    <div class="counter-offer-details" style="background: var(--bg-secondary); border-radius: 12px; padding: 20px; margin-bottom: 20px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                            <div>
                                <span style="color: var(--text-muted); font-size: 13px;">Your Bid</span>
                                <div style="font-size: 24px; font-weight: 700; color: var(--text-secondary); text-decoration: line-through;">${originalAmount} QAR</div>
                            </div>
                            <div style="font-size: 24px; color: var(--warning);">→</div>
                            <div>
                                <span style="color: var(--text-muted); font-size: 13px;">Customer Offer</span>
                                <div style="font-size: 28px; font-weight: 800; color: var(--success);">${proposedAmount} QAR</div>
                            </div>
                        </div>
                        <div style="background: var(--warning); color: #000; padding: 8px 12px; border-radius: 8px; text-align: center;">
                            <strong>Difference: ${savings} QAR (${savingsPercent}% less)</strong>
                        </div>
                        ${message ? `<div style="margin-top: 16px; padding: 12px; background: var(--bg-tertiary); border-radius: 8px; font-style: italic; color: var(--text-secondary);">"${escapeHTML(message)}"</div>` : ''}
                    </div>
                    
                    <div style="display: flex; gap: 12px; margin-bottom: 20px;">
                        <button onclick="respondToCounterOffer('${counterOfferId}', 'accept')" class="btn-success" style="flex: 1; padding: 16px; font-size: 16px; font-weight: 700; border-radius: 12px; cursor: pointer; border: none;">
                            <i class="bi bi-check-circle-fill"></i> Accept ${proposedAmount} QAR
                        </button>
                        <button onclick="respondToCounterOffer('${counterOfferId}', 'reject')" class="btn-danger" style="flex: 1; padding: 16px; font-size: 16px; font-weight: 700; border-radius: 12px; cursor: pointer; border: none;">
                            <i class="bi bi-x-circle-fill"></i> Reject Offer
                        </button>
                    </div>
                    
                    <div style="text-align: center;">
                        <button onclick="openCounterCounterModal('${counterOfferId}', ${proposedAmount})" style="background: var(--accent); color: white; padding: 12px 24px; border-radius: 8px; border: none; cursor: pointer; font-weight: 600;">
                            <i class="bi bi-arrow-repeat"></i> Counter with Different Price
                        </button>
                    </div>
                </div>
            </div>
...
function closeCounterRespondModal() {
    const modal = document.getElementById('counterRespondModal');
 ...
 */
async function respondToCounterOffer(counterOfferId, action) {
    try {
        const res = await fetch(`${API_URL}/negotiations/counter-offers/${counterOfferId}/respond`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
        ...
        if (res.ok) {
            closeCounterRespondModal();
            showToast(action === 'accept' ? 'Counter-offer accepted! Order created.' : 'Counter-offer rejected.',
        ...
        console.error('Counter-offer response error:', err);
        showToast('Connection error', 'error');
    }
}

/**
 * Open modal to make a counter-counter offer
 */
function openCounterCounterModal(originalCounterId, currentAmount) {
    closeCounterRespondModal();

    const modalHtml = `
        <div id="counterCounterModal" class="modal" style="display: flex;">
            <div class="modal-content" style="max-width: 400px;">
                <div class="modal-header">
                    <h3><i class="bi bi-tag-fill text-primary"></i> Your Counter-Offer</h3>
                    <button onclick="document.getElementById('counterCounterModal').remove()" class="close-btn">&times;</button>
                </div>
                <div class="modal-body">
                    <p style="color: var(--text-secondary); margin-bottom: 16px;">Customer offered <strong>${currentAmount} QAR</strong>. Enter your counter-price:</p>
                    
                    <div class="input-group" style="margin-bottom: 20px;">
                        <label>Your Price (QAR)</label>
                        <input type="number" id="counterCounterAmount" 
                               value="${Math.round((currentAmount + Number(currentAmount) * 0.1))}" 
                               min="1" step="5" 
                    ...
                    <div class="input-group" style="margin-bottom: 20px;">
                        ...
                        <textarea id="counterCounterMessage" rows="2" placeholder="Explain your price..."></textarea>
                    </div>
                    
                    <button onclick="submitCounterCounter('${originalCounterId}')" class="btn-primary" style="width: 100%; padding: 14px; font-size: 16px; font-weight: 700;">
                        <i class="bi bi-send-fill"></i> Send Counter-Offer
                    </button>
                </div>
            </div>
        </div>
...
async function submitCounterCounter(originalCounterId) {
    const amount = parseFloat(document.getElementById('counterCounterAmount').value);
    const message = document.getElementById('counterCounterMessage').value;
    ...
    try {
        const res = await fetch(`${API_URL}/negotiations/counter-offers/${originalCounterId}/counter`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ proposed_amount: amount, message })
        });

        const data = await res.json();

        if (res.ok) {
            document.getElementById('counterCounterModal').remove();
        ...
        console.error('Counter-counter submission error:', err);
        showToast('Connection error', 'error');
    }
}
 ...
 */
async function loadPendingDisputes() {
    // Disputes are handled via orders - no separate load needed
    // Just ensure orders are loaded which shows dispute indicators
    console.log('[Dashboard] Pending disputes check - handled via loadOrders()');
}

/**
 * Update dispute badge counter in nav
 */
function updateDisputeBadge() {
    const badge = document.getElementById('disputeBadge');
    if (badge && pendingDisputes.length > 0) {
        badge.textContent = pendingDisputes.length;
