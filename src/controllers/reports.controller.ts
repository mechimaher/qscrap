...
export const getOrdersReport = async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.json({
            ...
            data: result.orders,
            total_records: result.orders.length
        });
    } catch (err) {
        logger.error('Orders report error', { error: (err as Error).message });
...
export const getRevenueReport = async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.json({
            ...
            generated_at: new Date().toISOString(),
            summary: result.summary,
            daily_breakdown: result.daily,
            top_garages: result.topGarages
        });
    } catch (err) {
        logger.error('Revenue report error', { error: (err as Error).message });
        res.status(500).json({ error: getErrorMessage(err) });
    }
};
...
export const getDisputesReport = async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.json({
            ...
            total_records: result.disputes.length
        });
    } catch (err) {
        logger.error('Disputes report error', { error: (err as Error).message });
        res.status(500).json({ error: getErrorMessage(err) });
    }
};
...
export const getDeliveriesReport = async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.json({
            ...
            total_drivers: result.drivers.length
        });
    } catch (err) {
        logger.error('Deliveries report error', { error: (err as Error).message });
        res.status(500).json({ error: getErrorMessage(err) });
    }
};
...
export const getGaragesReport = async (req: AuthRequest, res: Response) => {
    try {
        ...
        logger.error('Garages report error', { error: (err as Error).message });
        res.status(500).json({ error: getErrorMessage(err) });
    }
};
