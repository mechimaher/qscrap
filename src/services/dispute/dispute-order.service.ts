...
export const DISPUTE_CONFIGS: Record<
    string,
    {
...
export class DisputeOrderService {
    ...
    async createDispute(
        customerId: string,
        data: { order_id: string; reason: string; description: string; photoUrls: string[] }
    ) {
        ...
        try {
            ...
            const orderResult = await client.query(
                `SELECT o.*, EXTRACT(EPOCH FROM (NOW() - COALESCE(o.delivered_at, o.updated_at))) / 3600 as hours_since_delivery FROM orders o WHERE o.order_id = $1`,
                [data.order_id]
            );
            ...
            if (order.hours_since_delivery > DISPUTE_WINDOW_HOURS) {
                throw new Error(
                    `Dispute window expired. You had ${DISPUTE_WINDOW_HOURS} hours after delivery to report issues.`
                );
            }

            const existingDispute = await client.query(`SELECT dispute_id FROM disputes WHERE order_id = $1`, [
                data.order_id
            ]);
            if (existingDispute.rows.length > 0) {
                throw new Error('A dispute already exists for this order');
            }
            if (!DISPUTE_CONFIGS[data.reason]) {
                throw new Error('Invalid dispute reason');
            }
            if (data.photoUrls.length > MAX_DISPUTE_PHOTOS) {
                throw new Error(`Maximum ${MAX_DISPUTE_PHOTOS} photos allowed per dispute`);
            }
            if (['damaged', 'wrong_part', 'not_as_described'].includes(data.reason) && data.photoUrls.length === 0) {
            ...
            const disputeResult = await client.query(
                `INSERT INTO disputes (order_id, customer_id, garage_id, reason, description, photo_urls, refund_amount, restocking_fee) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING dispute_id, created_at`,
                [
                    data.order_id,
                    customerId,
                    order.garage_id,
            ...
            await client.query(`UPDATE orders SET order_status = 'disputed', updated_at = NOW() WHERE order_id = $1`, [
                data.order_id
            ]);
    ...
    async getMyDisputes(userId: string, userType: string, params: { page?: number; limit?: number; status?: string }) {
        ...
        const result = await this.pool.query(
            `SELECT d.*, o.order_number, o.part_price, o.total_amount, pr.car_make, pr.car_model, pr.part_description FROM disputes d JOIN orders o ON d.order_id = o.order_id JOIN part_requests pr ON o.request_id = pr.request_id ${whereClause} ORDER BY d.created_at DESC LIMIT $${paramIndex++} OFFSET $${paramIndex}`,
    ...
    async getDisputeDetails(disputeId: string, userId: string) {
        const result = await this.pool.query(
            `SELECT d.*, o.order_number, o.part_price, o.total_amount, o.delivery_fee, pr.car_make, pr.car_model, pr.part_description, u.full_name as customer_name, g.garage_name FROM disputes d JOIN orders o ON d.order_id = o.order_id JOIN part_requests pr ON o.request_id = pr.request_id JOIN users u ON d.customer_id = u.user_id JOIN garages g ON d.garage_id = g.garage_id WHERE d.dispute_id = $1 AND (d.customer_id = $2 OR d.garage_id = $2)`,
            [disputeId, userId]
        );
    ...
    async garageRespond(disputeId: string, garageId: string, responseMessage: string) {
        ...
        try {
            await client.query('BEGIN');
            const disputeResult = await client.query(
                `SELECT d.*, o.order_number, o.customer_id FROM disputes d JOIN orders o ON d.order_id = o.order_id WHERE d.dispute_id = $1 FOR UPDATE`,
                [disputeId]
            );
            ...
            await client.query(
                `UPDATE disputes SET status = 'under_review', garage_response = $2, updated_at = NOW() WHERE dispute_id = $1`,
                [disputeId, responseMessage]
            );
            await client.query('COMMIT');
            return { dispute };
        } catch (err) {
    ...
    async autoResolveDisputes() {
        const result = await this.pool.query(
            `UPDATE disputes SET status = 'resolved', resolution = 'refund_approved', resolved_by = 'platform_auto', resolved_at = NOW() WHERE status = 'contested' AND created_at < NOW() - INTERVAL '48 hours' RETURNING dispute_id, order_id, customer_id, garage_id, refund_amount`
        );
        for (const dispute of result.rows) {
            await this.pool.query(`UPDATE orders SET order_status = 'refunded' WHERE order_id = $1`, [
                dispute.order_id
            ]);
        }
        return result.rows;
