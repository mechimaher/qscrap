...
export class SearchService {
    ...
    async universalSearch(
        ...
        if (userType === 'admin' && (!searchType || searchType === 'users')) {
            const userResult = await this.pool.query(
                `SELECT user_id, full_name, email, phone_number, user_type, is_active, created_at FROM users WHERE (full_name ILIKE $1 OR email ILIKE $1 OR phone_number ILIKE $1) ORDER BY created_at DESC LIMIT $2`,
        ...
        if (!searchType || searchType === 'requests') {
            ...
            if (userType === 'customer') {
                ...
                requestQuery += ` AND (pr.status = 'pending' OR EXISTS (SELECT 1 FROM bids b WHERE b.request_id = pr.request_id AND b.garage_id = $2))`;
                params.push(userId);
            }
        ...
        if (userType === 'admin' && (!searchType || searchType === 'disputes')) {
            const disputeResult = await this.pool.query(
                `SELECT d.dispute_id, d.reason, d.description, d.status, d.created_at, o.order_number, u.full_name as customer_name FROM disputes d JOIN orders o ON d.order_id = o.order_id JOIN users u ON d.customer_id = u.user_id WHERE (d.reason ILIKE $1 OR d.description ILIKE $1 OR o.order_number ILIKE $1 OR u.full_name ILIKE $1) ORDER BY d.created_at DESC LIMIT $2`,
                [searchTerm, maxResults]
            );
    ...
    async getSuggestions(userId: string, userType: string, query: string) {
        ...
        const partResult = await this.pool.query(
            partQuery,
            userType === 'customer' ? [searchTerm, userId] : [searchTerm]
        );
