...
export class SubscriptionService {
    ...
    async getSubscriptionPlans() {
        ...
        const result = await this.pool.query(
            `SELECT plan_id, plan_code, plan_name, plan_name_ar, monthly_fee, commission_rate, max_bids_per_month, features, is_featured FROM subscription_plans WHERE is_active = true AND plan_code != 'demo' ORDER BY monthly_fee ASC, display_order ASC`
        );
        return result.rows;
    }

    async getMySubscription(garageId: string) {
        const bidCountResult = await this.pool.query(
            `SELECT COUNT(*) as bids_this_month FROM bids WHERE garage_id = $1 AND created_at >= DATE_TRUNC('month', CURRENT_DATE)`,
            [garageId]
        );
        ...
        const pendingRequest = await this.pool.query(
            `SELECT scr.*, sp.plan_name as target_plan_name FROM subscription_change_requests scr JOIN subscription_plans sp ON scr.to_plan_id = sp.plan_id WHERE scr.garage_id = $1 AND scr.status = 'pending'`,
            [garageId]
        );
        ...
        if (result.rows.length > 0) {
            ...
            return {
                subscription: { ...sub, bids_used_this_cycle: bidsThisMonth, bids_remaining: bidsRemaining },
                pending_request: pendingRequest.rows[0] || null
            };
        }

        const garageResult = await this.pool.query(
            `SELECT approval_status, demo_expires_at FROM garages WHERE garage_id = $1`,
            [garageId]
        );
    ...
    async subscribeToPlan(garageId: string, planCode: string, paymentMethod?: string) {
        ...
        try {
            ...
            const existingSub = await client.query(
                `SELECT subscription_id FROM garage_subscriptions WHERE garage_id = $1 AND status IN ('active', 'trial')`,
                [garageId]
            );
            ...
            const subResult = await client.query(
                `INSERT INTO garage_subscriptions (garage_id, plan_id, status, billing_cycle_start, billing_cycle_end, next_billing_date) VALUES ($1, $2, 'active', $3, $4, $4) RETURNING subscription_id`,
                [garageId, plan.plan_id, today.toISOString().split('T')[0], cycleEnd.toISOString().split('T')[0]]
            );
            await client.query(
                `INSERT INTO subscription_payments (subscription_id, amount, payment_method, payment_status, processed_at) VALUES ($1, $2, $3, 'completed', NOW())`,
                [subResult.rows[0].subscription_id, plan.monthly_fee, paymentMethod || 'card']
            );
            await client.query('COMMIT');

            return {
                subscription_id: subResult.rows[0].subscription_id,
    ...
    async changePlan(garageId: string, planId: string, reason?: string) {
        ...
        try {
            ...
            const currentSub = await client.query(
                `SELECT plan_id, monthly_fee FROM garage_subscriptions JOIN subscription_plans USING (plan_id) WHERE garage_id = $1 AND status IN ('active', 'trial')`,
                [garageId]
            );
            ...
            const pendingCheck = await client.query(
                `SELECT request_id, to_plan_id FROM subscription_change_requests WHERE garage_id = $1 AND status = 'pending'`,
                [garageId]
            );
            if (pendingCheck.rows.length > 0) {
                if (pendingCheck.rows[0].to_plan_id === currentPlanId) {
                    await client.query(
                        `UPDATE subscription_change_requests SET status = 'approved', updated_at = NOW(), admin_notes = 'Auto-resolved' WHERE request_id = $1`,
                        [pendingCheck.rows[0].request_id]
                    );
                } else {
            ...
            const newPlan = await client.query(
                `SELECT plan_id, monthly_fee, plan_name FROM subscription_plans WHERE plan_id = $1`,
                [planId]
            );
    ...
    async cancelSubscription(garageId: string, reason?: string) {
        ...
        try {
            await client.query('BEGIN');
            const result = await client.query(
                `UPDATE garage_subscriptions SET status = 'cancelled', cancelled_at = NOW(), cancellation_reason = $2, auto_renew = false, updated_at = NOW() WHERE garage_id = $1 AND status IN ('active', 'trial') RETURNING subscription_id, billing_cycle_end`,
                [garageId, reason]
            );
            if (result.rows.length === 0) {
    ...
    async getPaymentHistory(garageId: string) {
        const result = await this.pool.query(
            `SELECT sp.*, gs.billing_cycle_start, gs.billing_cycle_end FROM subscription_payments sp JOIN garage_subscriptions gs ON sp.subscription_id = gs.subscription_id WHERE gs.garage_id = $1 ORDER BY sp.created_at DESC LIMIT 12`,
            [garageId]
        );
        return result.rows;
    }
    ...
    async confirmUpgradePayment(paymentIntentId: string): Promise<{ success: boolean; requestId?: string }> {
        ...
        try {
            ...
            logger.info('Upgrade payment confirmed & executed', {
                garageName: request.garage_name,
                planName: request.plan_name
            });
