...
export class DashboardUrgentService {
    ...
    async getCustomerUrgentActions(customerId: string) {
        ...
        pendingPayments.rows.forEach((order) => {
            urgentActions.push({
                ...
                action: 'Complete payment to proceed with order'
            });
        });
        ...
        const deliveryConfirmations = await this.pool.query(
            ...
            ORDER BY actual_delivery_at DESC LIMIT 5`,
            [customerId]
        );
        ...
        deliveryConfirmations.rows.forEach((order) => {
            urgentActions.push({
                ...
                order_id: order.order_id,
                order_number: order.order_number,
                delivered_at: order.actual_delivery_at,
                address: order.delivery_address,
                action: 'Confirm delivery to complete order'
            });
        });
        ...
        const expiringBids = await this.pool.query(
            ...
            ORDER BY pr.expires_at ASC LIMIT 5`,
            [customerId]
        );
        ...
        expiringBids.rows.forEach((bid) => {
            urgentActions.push({
                ...
                garage_name: bid.garage_name,
                amount: bid.bid_amount,
                part_description: bid.part_description,
                vehicle: `${bid.car_year} ${bid.car_make} ${bid.car_model}`,
                expires_in_seconds: Math.floor(bid.seconds_until_expiry),
                action: 'Review bid before it expires'
            });
        });
        ...
        const pendingCounterOffers = await this.pool.query(
            ...
            ORDER BY co.expires_at ASC LIMIT 5`,
            [customerId]
        );
        ...
        pendingCounterOffers.rows.forEach((offer) => {
            urgentActions.push({
                ...
                bid_id: offer.bid_id,
                garage_name: offer.garage_name,
                new_amount: offer.proposed_amount,
                part_description: offer.part_description,
                expires_in_seconds: Math.floor(offer.seconds_until_expiry),
    ...
    async getCustomerContextualData(customerId: string) {
        ...
        const [unreadBids, activeOrders, moneySaved, loyaltyPoints] = await Promise.all([
            this.pool.query(
                `SELECT COUNT(DISTINCT b.bid_id) as count FROM bids b JOIN part_requests pr ON b.request_id = pr.request_id WHERE pr.customer_id = $1 AND b.status = 'pending' AND pr.status = 'active'`,
                [customerId]
            ),
            this.pool.query(
                `SELECT COUNT(*) as count FROM orders WHERE customer_id = $1 AND order_status NOT IN ('completed', 'delivered', 'cancelled_by_customer', 'cancelled_by_garage', 'cancelled_by_ops', 'refunded')`,
                [customerId]
            ),
            this.pool.query(
                `SELECT COALESCE(SUM(total_amount), 0) as total_spent, COUNT(*) as order_count FROM orders WHERE customer_id = $1 AND order_status = 'completed' AND completed_at >= DATE_TRUNC('month', CURRENT_DATE)`,
                [customerId]
            ),
            this.pool.query(
                `SELECT COALESCE(points_balance, 0) as balance FROM customer_rewards WHERE customer_id = $1`,
                [customerId]
            )
        ]);
