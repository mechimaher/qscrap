...
export class DashboardService {
    ...
    async getGarageStats(garageId: string) {
        const [pendingBids, activeBids, activeOrders, completedOrders, revenue, profile] = await Promise.all([
            this.pool.query(`SELECT COUNT(*) as count FROM bids WHERE garage_id = $1 AND status = 'pending'`, [
                garageId
            ]),
            this.pool.query(
                `SELECT COUNT(*) as count FROM bids WHERE garage_id = $1 AND status = 'accepted' AND created_at >= DATE_TRUNC('month', CURRENT_DATE)`,
                [garageId]
            ),
            this.pool.query(
                `SELECT COUNT(*) as count FROM orders WHERE garage_id = $1 AND order_status NOT IN ('completed', 'delivered', 'cancelled_by_customer', 'cancelled_by_garage', 'cancelled_by_ops', 'refunded')`,
                [garageId]
            ),
            this.pool.query(
                `SELECT COUNT(*) as count FROM orders WHERE garage_id = $1 AND order_status = 'completed' AND completed_at >= DATE_TRUNC('month', CURRENT_DATE)`,
                [garageId]
            ),
            this.pool.query(
                `SELECT COALESCE(SUM(garage_payout_amount), 0) as total FROM orders WHERE garage_id = $1 AND order_status = 'completed' AND completed_at >= DATE_TRUNC('month', CURRENT_DATE)`,
                [garageId]
            ),
            this.pool.query(
                `SELECT g.*, gs.plan_id, gs.status as subscription_status, sp.plan_name FROM garages g LEFT JOIN garage_subscriptions gs ON g.garage_id = gs.garage_id AND gs.status IN ('active', 'trial') LEFT JOIN subscription_plans sp ON gs.plan_id = sp.plan_id WHERE g.garage_id = $1`,
                [garageId]
            )
        ]);
    ...
    async updateGarageBusinessDetails(
        garageId: string,
        data: {
            cr_number?: string;
    ...
    async updateGarageSpecialization(
        garageId: string,
        data: { supplier_type?: string; specialized_brands?: string[]; all_brands?: boolean }
    ) {
        const result = await this.pool.query(
            `
    ...
    async updateGarageLocation(garageId: string, lat: number, lng: number, address?: string) {
        ...
        const result = await this.pool.query(
            `UPDATE garages SET ${updateFields.join(', ')} WHERE garage_id = $${params.length} RETURNING garage_id, garage_name, address, location_lat, location_lng`,
            params
        );
        return result.rows[0];
    }
    ...
    async getCustomerStats(customerId: string) {
        const [activeRequests, totalOrders, inProgressOrders, awaitingConfirmation] = await Promise.all([
            ...
            this.pool.query(
                `SELECT COUNT(*) as count FROM orders WHERE customer_id = $1 AND order_status IN ('confirmed', 'preparing', 'ready_for_pickup', 'collected', 'in_transit')`,
                [customerId]
            ),
            this.pool.query(
                `SELECT COUNT(*) as count FROM orders WHERE customer_id = $1 AND order_status = 'delivered'`,
                [customerId]
            )
        ]);
        return {
    ...
    async getCustomerProfile(customerId: string) {
        const [userResult, statsResult, addressesResult] = await Promise.all([
            this.pool.query(
                `SELECT user_id, full_name, phone_number, email, created_at FROM users WHERE user_id = $1`,
                [customerId]
            ),
            this.pool.query(
                `SELECT (SELECT COUNT(*) FROM part_requests WHERE customer_id = $1) as total_requests, (SELECT COUNT(*) FROM orders WHERE customer_id = $1) as total_orders, (SELECT COUNT(*) FROM orders WHERE customer_id = $1 AND order_status = 'completed') as completed_orders, (SELECT COUNT(*) FROM order_reviews WHERE customer_id = $1) as reviews_given`,
                [customerId]
            ),
            this.pool.query(
                `SELECT address_id, label, address_line, area, city, delivery_notes, is_default FROM customer_addresses WHERE customer_id = $1 ORDER BY is_default DESC, created_at DESC`,
                [customerId]
            )
        ]);
        return {
    ...
    async updateCustomerProfile(customerId: string, data: { full_name?: string; email?: string }) {
        const result = await this.pool.query(
            `UPDATE users SET full_name = $1, email = $2, updated_at = NOW() WHERE user_id = $3 RETURNING user_id, full_name, phone_number, email`,
            [data.full_name, data.email, customerId]
        );
        return result.rows[0];
    }
    ...
    async addAddress(
        customerId: string,
        data: { label?: string; address_line: string; area: string; delivery_notes?: string; is_default?: boolean }
    ) {
        if (data.is_default) {
            await this.pool.query(`UPDATE customer_addresses SET is_default = false WHERE customer_id = $1`, [
                customerId
            ]);
        }
        const result = await this.pool.query(
            `INSERT INTO customer_addresses (customer_id, label, address_line, area, delivery_notes, is_default) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
            [
                customerId,
                data.label || 'Home',
                data.address_line,
    ...
    async setDefaultAddress(customerId: string, addressId: string) {
        ...
        const result = await this.pool.query(
            `UPDATE customer_addresses SET is_default = true WHERE address_id = $1 AND customer_id = $2 RETURNING *`,
            [addressId, customerId]
        );
        return result.rows[0];
    }
    ...
    async getNotifications(userId: string) {
        const result = await this.pool.query(
            `SELECT notification_id, title, body as message, notification_type as type, is_read, created_at, data->'related_id' as related_id, data FROM notifications WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50`,
            [userId]
        );
        return result.rows;
    }
    ...
    async markNotificationRead(userId: string, notificationId: string) {
        await this.pool.query(`UPDATE notifications SET is_read = true WHERE notification_id = $1 AND user_id = $2`, [
            notificationId,
            userId
        ]);
    }
    ...
    async markAllNotificationsRead(userId: string) {
        await this.pool.query(`UPDATE notifications SET is_read = true WHERE user_id = $1 AND is_read = false`, [
            userId
        ]);
    }

    async deleteNotification(userId: string, notificationId: string) {
        const result = await this.pool.query(`DELETE FROM notifications WHERE notification_id = $1 AND user_id = $2`, [
            notificationId,
            userId
        ]);
        return result.rowCount! > 0;
