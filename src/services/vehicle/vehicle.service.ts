...
export class VehicleService {
    ...
    async getMyVehicles(customerId: string) {
        const result = await this.pool.query(
            `SELECT vehicle_id, car_make, car_model, car_year, vin_number, front_image_url, rear_image_url, nickname, is_primary, last_used_at, request_count, created_at FROM customer_vehicles WHERE customer_id = $1 ORDER BY is_primary DESC, last_used_at DESC NULLS LAST LIMIT 10`,
            [customerId]
        );
        return result.rows;
    }
    ...
    async saveVehicle(
        ...
        if (data.is_primary) {
            await this.pool.query(
                `UPDATE customer_vehicles SET is_primary = false, updated_at = NOW() WHERE customer_id = $1`,
                [customerId]
            );
        }
        if (data.vin_number) {
            const existing = await this.pool.query(
                `SELECT vehicle_id FROM customer_vehicles WHERE customer_id = $1 AND vin_number = $2`,
                [customerId, data.vin_number]
            );
            if (existing.rows.length > 0) {
                const updated = await this.pool.query(
                    `UPDATE customer_vehicles SET car_make = $2, car_model = $3, car_year = $4, nickname = COALESCE($5, nickname), front_image_url = COALESCE($6, front_image_url), rear_image_url = COALESCE($7, rear_image_url), is_primary = COALESCE($8, is_primary), updated_at = NOW() WHERE vehicle_id = $9 RETURNING *`,
                    [
                        customerId,
                        data.car_make,
                        data.car_model,
        ...
        const result = await this.pool.query(
            `INSERT INTO customer_vehicles (customer_id, car_make, car_model, car_year, vin_number, nickname, front_image_url, rear_image_url, is_primary) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`,
            [
                customerId,
                data.car_make,
                data.car_model,
    ...
    async updateVehicle(
        ...
        if (data.is_primary) {
            await this.pool.query(
                `UPDATE customer_vehicles SET is_primary = false, updated_at = NOW() WHERE customer_id = $1 AND vehicle_id != $2`,
                [customerId, vehicleId]
            );
        }
        const result = await this.pool.query(
            `UPDATE customer_vehicles SET nickname = COALESCE($3, nickname), is_primary = COALESCE($4, is_primary), vin_number = COALESCE($5, vin_number), updated_at = NOW() WHERE vehicle_id = $1 AND customer_id = $2 RETURNING *`,
            [vehicleId, customerId, data.nickname, data.is_primary, data.vin_number]
        );
    ...
    async deleteVehicle(customerId: string, vehicleId: string) {
        ...
        if (activeRequestsCount > 0 || activeOrdersCount > 0) {
            ...
            if (activeRequestsCount > 0) {
                errors.push(`${activeRequestsCount} active request${activeRequestsCount > 1 ? 's' : ''}`);
            }
            if (activeOrdersCount > 0) {
                errors.push(`${activeOrdersCount} active order${activeOrdersCount > 1 ? 's' : ''}`);
            }
    ...
    async autoSaveVehicle(
        ...
        if (vinNumber) {
            ...
            existing = await this.pool.query(
                `SELECT vehicle_id FROM customer_vehicles WHERE customer_id = $1 AND car_make = $2 AND car_model = $3 AND car_year = $4 AND vin_number IS NULL`,
                [customerId, carMake, carModel, carYear]
            );
        }
        if (existing.rows.length > 0) {
            await this.pool.query(
                `UPDATE customer_vehicles SET last_used_at = NOW(), front_image_url = COALESCE($2, front_image_url), rear_image_url = COALESCE($3, rear_image_url), updated_at = NOW() WHERE vehicle_id = $1`,
                [existing.rows[0].vehicle_id, frontImageUrl, rearImageUrl]
            );
            return existing.rows[0].vehicle_id;
        }
        const result = await this.pool.query(
            `INSERT INTO customer_vehicles (customer_id, car_make, car_model, car_year, vin_number, front_image_url, rear_image_url) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING vehicle_id`,
