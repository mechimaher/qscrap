...
export class AddressService {
    ...
    async addAddress(userId: string, data: AddressData) {
        ...
        try {
            ...
            const result = await client.query(
                `INSERT INTO user_addresses (user_id, label, address_text, latitude, longitude, is_default) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
                [userId, data.label, data.address_text, data.latitude || null, data.longitude || null, shouldBeDefault]
            );
    ...
    async updateAddress(userId: string, addressId: string, data: AddressData) {
        ...
        try {
            await client.query('BEGIN');
            const check = await client.query('SELECT 1 FROM user_addresses WHERE address_id = $1 AND user_id = $2', [
                addressId,
                userId
            ]);
            if (check.rowCount === 0) {
            ...
            const result = await client.query(
                `UPDATE user_addresses SET label = $1, address_text = $2, latitude = $3, longitude = $4, is_default = COALESCE($5, is_default), updated_at = NOW() WHERE address_id = $6 AND user_id = $7 RETURNING *`,
                [
                    data.label,
                    data.address_text,
                    data.latitude || null,
