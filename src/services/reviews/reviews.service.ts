...
export class ReviewsService {
    ...
    async getMyReviews(garageId: string, limit: number, offset: number) {
        ...
        const statsResult = await this.pool.query(
            `SELECT COUNT(*) as total_reviews, ROUND(AVG(overall_rating)::numeric, 1) as avg_rating FROM order_reviews WHERE garage_id = $1 AND moderation_status = 'approved'`,
            [garageId]
        );
        return {
            reviews: result.rows,
    ...
    async getPendingReviews(limit: number, offset: number) {
        const result = await this.pool.query(
            `SELECT r.review_id, r.overall_rating, r.part_quality_rating, r.communication_rating, r.delivery_rating, r.review_text, r.created_at, r.moderation_status, u.full_name as customer_name, g.garage_name, o.order_number FROM order_reviews r JOIN users u ON r.customer_id = u.user_id JOIN garages g ON r.garage_id = g.garage_id LEFT JOIN orders o ON r.order_id = o.order_id WHERE r.moderation_status = 'pending' ORDER BY r.created_at ASC LIMIT $1 OFFSET $2`,
            [limit, offset]
        );
    ...
    async moderateReview(reviewId: string, action: string, moderatorId: string, rejectionReason?: string) {
        ...
        const result = await this.pool.query(
            `UPDATE order_reviews SET moderation_status = $1, moderated_by = $2, moderated_at = NOW(), rejection_reason = $3, is_visible = $4 WHERE review_id = $5 RETURNING review_id, moderation_status, garage_id`,
            [newStatus, moderatorId, rejectionReason || null, action === 'approve', reviewId]
        );
        ...
        if (action === 'approve') {
            await this.pool.query(
                `UPDATE garages g SET rating_average = (SELECT ROUND(AVG(overall_rating)::numeric, 2) FROM order_reviews WHERE garage_id = g.garage_id AND moderation_status = 'approved'), rating_count = (SELECT COUNT(*) FROM order_reviews WHERE garage_id = g.garage_id AND moderation_status = 'approved') WHERE g.garage_id = $1`,
