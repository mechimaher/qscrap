...
export class ReportsService {
    ...
    async getOrdersReport(fromDate: string, toDate: string, status?: string) {
        ...
        if (status && status !== 'all') {
            statusFilter = `AND o.order_status = $${paramIndex++}`;
            params.push(status);
        }
        const ordersResult = await this.pool.query(
            `SELECT o.order_id, o.order_number, o.order_status, o.payment_status, o.part_price, o.platform_fee, o.delivery_fee, o.total_amount, o.garage_payout_amount, o.created_at, o.completed_at, cu.full_name as customer_name, cu.phone_number as customer_phone, g.garage_name, pr.part_description, pr.car_make, pr.car_model, pr.car_year FROM orders o JOIN users cu ON o.customer_id = cu.user_id JOIN garages g ON o.garage_id = g.garage_id LEFT JOIN part_requests pr ON o.request_id = pr.request_id WHERE o.created_at >= $1 AND o.created_at <= $2::date + interval '1 day' ${statusFilter} ORDER BY o.created_at DESC`,
            params
        );
        ...
        if (status && status !== 'all') {
            summaryStatusFilter = `AND order_status = $${summaryParamIndex++}`;
            summaryParams.push(status);
        }
        const summaryResult = await this.pool.query(
            `SELECT COUNT(*) as total_orders, COUNT(*) FILTER (WHERE order_status = 'completed') as completed_orders, COUNT(*) FILTER (WHERE order_status IN ('cancelled_by_customer', 'cancelled_by_garage', 'cancelled_by_ops')) as cancelled_orders, COUNT(*) FILTER (WHERE order_status = 'disputed') as disputed_orders, COALESCE(SUM(total_amount), 0) as total_revenue, COALESCE(SUM(platform_fee), 0) as total_platform_fees, COALESCE(SUM(garage_payout_amount), 0) as total_garage_payouts, COALESCE(AVG(total_amount), 0) as average_order_value FROM orders WHERE created_at >= $1 AND created_at <= $2::date + interval '1 day' ${summaryStatusFilter}`,
            summaryParams
        );
    ...
    async getDisputesReport(fromDate: string, toDate: string) {
        const disputesResult = await this.pool.query(
            `SELECT d.dispute_id, d.reason, d.description, d.resolution, d.refund_amount, d.status, d.created_at, d.resolved_at, o.order_number, o.total_amount, cu.full_name as customer_name, g.garage_name FROM disputes d JOIN orders o ON d.order_id = o.order_id JOIN users cu ON d.customer_id = cu.user_id JOIN garages g ON d.garage_id = g.garage_id WHERE d.created_at >= $1 AND d.created_at <= $2::date + interval '1 day' ORDER BY d.created_at DESC`,
            [fromDate, toDate]
        );
        const summaryResult = await this.pool.query(
            `SELECT COUNT(*) as total_disputes, COUNT(*) FILTER (WHERE status = 'resolved' OR status = 'accepted') as resolved, COUNT(*) FILTER (WHERE status = 'pending') as pending, COALESCE(SUM(refund_amount) FILTER (WHERE status IN ('resolved', 'accepted')), 0) as total_refunded, COUNT(*) FILTER (WHERE resolution = 'refund_approved') as full_refunds, COUNT(*) FILTER (WHERE resolution = 'partial_refund') as partial_refunds FROM disputes WHERE created_at >= $1 AND created_at <= $2::date + interval '1 day'`,
            [fromDate, toDate]
        );
