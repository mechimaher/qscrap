...
describe('Payment Service', () => {
    ...
    beforeAll(async () => {
        ...
        await pool.query('DELETE FROM payment_refunds WHERE intent_id IN (SELECT intent_id FROM payment_intents WHERE order_id IN (SELECT order_id FROM orders WHERE bid_id = $1))', [testBidId]);
        await pool.query('DELETE FROM payment_intents WHERE order_id IN (SELECT order_id FROM orders WHERE bid_id = $1)', [testBidId]);
    ...
    describe('createDeliveryFeeDeposit', () => {
        it('should create delivery fee deposit intent with correct amount', async () => {
            const result = await paymentService.createDeliveryFeeDeposit(
                testOrderId!,
                testCustomerId,
                20.00,
                'QAR'
    ...
    describe('createFullPaymentIntent', () => {
        ...
        it('should set payment_method = card_full on order', async () => {
            ...
            const orderResult = await pool.query(
                'SELECT payment_method FROM orders WHERE order_id = $1',
                [fullPaymentOrderId]
            );
    ...
    describe('confirmDepositPayment', () => {
        ...
        it('should return true and update order when payment succeeded', async () => {
            ...
            const depositResult = await paymentService.createDeliveryFeeDeposit(newOrderId, testCustomerId, 20.00, 'QAR');
    ...
    describe('processCancellationRefund', () => {
        ...
        afterAll(async () => {
            if (refundOrderId) {
                await pool.query('DELETE FROM payment_refunds WHERE intent_id IN (SELECT intent_id FROM payment_intents WHERE order_id = $1)', [refundOrderId]);
        ...
        it('should return not_applicable when no deposit was paid', async () => {
            ...
            const result = await paymentService.processCancellationRefund(
                refundOrderId,
                0,
                'customer_request'
            );
        ...
        it('should process full refund when no fee retention', async () => {
            ...
            // Create deposit
            await paymentService.createDeliveryFeeDeposit(newOrderId, testCustomerId, 20.00, 'QAR');
            ...
            // Process full refund
            const refundResult = await paymentService.processCancellationRefund(
                newOrderId,
                0,
                'customer_request'
            );
    ...
    describe('Edge Cases', () => {
        it('should handle duplicate deposit creation gracefully', async () => {
            ...
            await expect(
                paymentService.createDeliveryFeeDeposit(testOrderId!, testCustomerId, 20.00, 'QAR')
