...
export class VINService {
    ...
    decodeVIN(vin: string): VehicleDetails | null {
        ...
        if (!make) {
            // Geographic Fallback
            if (normalizedVin.startsWith('J')) {
                make = 'Japanese Make';
            } else if (normalizedVin.startsWith('K')) {
                make = 'Korean Make';
            } else if (normalizedVin.startsWith('W')) {
                make = 'German Make';
            } else if (
                normalizedVin.startsWith('1') ||
                normalizedVin.startsWith('4') ||
                normalizedVin.startsWith('5')
            ) {
                make = 'American Make';
            } else {
                return null;
