...
export const loginSchema = z.object({
    phone_number: z.string().min(3, 'Identifier must be at least 3 characters').max(100, 'Identifier too long'),
...
export const registerCustomerSchema = z.object({
    ...
    phone_number: z.string().min(8, 'Phone number must be at least 8 characters').max(20, 'Phone number too long'),
    password: z.string().min(6, 'Password must be at least 6 characters').max(100, 'Password too long')
});

export const registerGarageSchema = z.object({
    garage_name: z.string().min(2, 'Garage name must be at least 2 characters').max(100, 'Garage name too long'),
    full_name: z.string().min(2, 'Owner name must be at least 2 characters').max(100, 'Owner name too long'),
    phone_number: z.string().min(8, 'Phone number must be at least 8 characters').max(20, 'Phone number too long'),
    email: z.string().email('Invalid email address').optional(),
    password: z.string().min(6, 'Password must be at least 6 characters').max(100, 'Password too long'),
    address: z.string().min(5, 'Address must be at least 5 characters').max(200, 'Address too long').optional(),
    cr_number: z.string().max(50, 'CR number too long').optional(),
    trade_license_number: z.string().max(50, 'Trade license number too long').optional(),
    supplier_type: z.enum(['used', 'new', 'both']).optional(),
    specialized_brands: z.array(z.string()).optional(),
    all_brands: z.boolean().optional()
});
...
export const createBidSchema = z.object({
    ...
    part_price: z.number().positive('Price must be positive').max(100000, 'Price too high'),
...
export const createRequestSchema = z.object({
    car_make: z.string().min(2, 'Car make must be at least 2 characters').max(50, 'Car make too long'),
    car_model: z.string().min(1, 'Car model is required').max(50, 'Car model too long'),
    car_year: z
        .number()
        .int('Year must be a whole number')
        .min(1970, 'Year too old')
        .max(new Date().getFullYear() + 1, 'Invalid year'),
    part_category: z.string().min(1, 'Category is required').max(50, 'Category too long'),
    part_description: z
        .string()
