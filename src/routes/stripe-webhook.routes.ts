...
async function handleSubscriptionUpgradePayment(intent: Stripe.PaymentIntent): Promise<void> {
    ...
    try {
        ...
        logger.info('Subscription upgrade payment confirmed & executed', {
...
async function handleSetupIntentSucceeded(setupIntent: Stripe.SetupIntent): Promise<void> {
    ...
    try {
        ...
        const existing = await pool.query('SELECT COUNT(*) FROM garage_payment_methods WHERE garage_id = $1', [
            garage_id
        ]);
...
async function handleChargeRefunded(charge: Stripe.Charge): Promise<void> {
    ...
    try {
        ...
        if (refundResult.rows.length > 0) {
            ...
            await pool.query(`UPDATE orders SET payment_status = 'refunded', updated_at = NOW() WHERE order_id = $1`, [
                orderId
            ]);
        }
    } catch (error: any) {
