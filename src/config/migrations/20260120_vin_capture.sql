...
BEGIN
    IF EXISTS (
        ...
        EXECUTE 'ALTER TABLE customer_vehicles ADD COLUMN IF NOT EXISTS vin_image_url VARCHAR(500)';
        EXECUTE 'COMMENT ON COLUMN customer_vehicles.vin_image_url IS ''Photo of registration card (Istimara) showing VIN number''';
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_customer_vehicles_vin ON customer_vehicles(vin_number) WHERE vin_number IS NOT NULL AND vin_number != ''''';
    END IF;
END $$;
