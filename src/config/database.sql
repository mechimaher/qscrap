...
\restrict Q9Qgn4rJj9ugOfX1BMK6i3mGjBj1ar4Jd26KVqhKEHrVSr6pweNlDe6xU2gTUcw
...
CREATE TABLE public.garage_products (
    ...
    created_at timestamp without time zone DEFAULT now(),
    updated_at timestamp without time zone DEFAULT now(),
    CONSTRAINT valid_condition CHECK (((condition)::text = ANY ((ARRAY['new'::character varying, 'used_excellent'::character varying, 'used_good'::character varying, 'used_fair'::character varying, 'refurbished'::character varying])::text[]))),
    CONSTRAINT valid_status CHECK (((status)::text = ANY ((ARRAY['draft'::character varying, 'active'::character varying, 'sold'::character varying, 'archived'::character varying])::text[])))
);
...
CREATE TABLE public.refunds (
    ...
    refund_reason text,
    processed_by uuid,
    CONSTRAINT refunds_refund_method_check CHECK (((refund_method)::text = ANY (ARRAY[('original_payment'::character varying)::text, ('wallet_credit'::character varying)::text, ('bank_transfer'::character varying)::text, ('cash'::character varying)::text]))),
    CONSTRAINT refunds_refund_status_check CHECK (((refund_status)::text = ANY (ARRAY[('pending'::character varying)::text, ('processing'::character varying)::text, ('completed'::character varying)::text, ('failed'::character varying)::text])))
);
...
--
-- Name: customer_addresses trg_update_customer_address_modtime; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_update_customer_address_modtime BEFORE UPDATE ON public.customer_addresses FOR EACH ROW EXECUTE FUNCTION public.update_address_modtime();
...
\unrestrict Q9Qgn4rJj9ugOfX1BMK6i3mGjBj1ar4Jd26KVqhKEHrVSr6pweNlDe6xU2gTUcw
